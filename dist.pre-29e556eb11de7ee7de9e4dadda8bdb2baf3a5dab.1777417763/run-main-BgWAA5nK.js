...
function loadConfigCli() {
	configCliPromise ??= import("./config-cli-DTy0ZcV1.js");
	return configCliPromise;
}
function loadModelsList() {
	modelsListPromise ??= import("./list-BGJiSGcj.js");
...
const routedCommandDefinitions = {
	health: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { healthCommand } = await import("./health-qVtpmWPC.js");
	...
	status: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			if (args.json) {
				const { statusJsonCommand } = await import("./status-json-BjguLiqE.js");
			...
			const { statusCommand } = await import("./status-C5hn6pTD.js");
	...
	"gateway-status": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { runDaemonStatus } = await import("./status-CNq3_I2P.js");
	...
	"agents-list": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { agentsListCommand } = await import("./agents-DBBAbhXQ.js");
	...
	"channels-list": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { channelsListCommand } = await import("./list-k9MAXJub.js");
	...
	"channels-status": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { channelsStatusCommand } = await import("./status-R8oLsLjt.js");
	...
	try {
		if (shouldUseRootHelpFastPath(normalizedArgv)) {
			...
			if (!outputPrecomputedRootHelpText()) {
				const { outputRootHelp } = await import("./root-help-BVVvua23.js");
		...
		if (primary && shouldRegisterPrimaryCommandOnly(parseArgv)) {
			...
			if (ctx) {
				const { registerCoreCliByName } = await import("./command-registry-CJSgW8Vg.js");
				await registerCoreCliByName(program, ctx, primary, parseArgv);
			}
			const { registerSubCliByName } = await import("./register.subclis-BEedwzMX.js");
