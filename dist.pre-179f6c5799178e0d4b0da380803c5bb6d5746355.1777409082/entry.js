...
function tryHandleRootVersionFastPath(argv, deps = {}) {
	...
	(deps.resolveVersion ?? (async () => {
		const [{ VERSION }, { resolveCommitHash }] = await Promise.all([import("./version-LMlsBH_W.js"), import("./git-commit-HPyuAR_9.js")]);
		return {
			VERSION,
			resolveCommitHash
		};
...
function tryHandleRootHelpFastPath(argv, deps = {}) {
	...
	import("./root-help-metadata-1kDR2AaT.js").then(async ({ outputPrecomputedRootHelpText }) => {
...
function runMainOrRootHelp(argv) {
	...
	import("./run-main-Bz_61F5z.js").then(({ runCli }) => runCli(argv)).catch((error) => {
