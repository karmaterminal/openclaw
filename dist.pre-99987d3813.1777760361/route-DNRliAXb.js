...
import { i as cliCommandCatalog, r as matchesCommandPath, t as resolveCliCommandPathPolicy } from "./command-path-policy-Cvup1Nuu.js";
import { n as ensureCliExecutionBootstrap, r as resolveCliExecutionStartupContext, t as applyCliExecutionStartupPresentation } from "./command-execution-startup-OWizUVl-.js";
...
function loadConfigCli() {
	configCliPromise ??= import("./config-cli-Dwufp4iL.js");
	return configCliPromise;
}
function loadModelsListCommand() {
	modelsListCommandPromise ??= import("./list.list-command-C5R5jatu.js");
	return modelsListCommandPromise;
}
function loadModelsStatusCommand() {
	modelsStatusCommandPromise ??= import("./list.status-command-Dv6Q5jEi.js");
...
const routedCommandDefinitions = {
	...
	status: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			if (args.json) {
				const { statusJsonCommand } = await import("./status-json-Bgjuc5ip.js");
			...
			const { statusCommand } = await import("./status-BCzPGuaz.js");
	...
	"gateway-status": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { runDaemonStatus } = await import("./status-D-Fh3K2a.js");
	...
	sessions: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { sessionsCommand } = await import("./sessions-CqzAtvKA.js");
	...
	"agents-list": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { agentsListCommand } = await import("./agents-Dygc0i6-.js");
	...
	"tasks-list": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { tasksListJsonCommand } = await import("./tasks-json-CWWRhv9o.js");
	...
	"tasks-audit": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { tasksAuditJsonCommand } = await import("./tasks-json-CWWRhv9o.js");
