...
import { i as normalizeEnv, t as isTruthyEnvValue } from "../env-CqfGXlE5.js";
...
import { i as shouldSkipPluginCommandRegistration, n as shouldRegisterPrimaryCommandOnly } from "../command-registration-policy-CAl2OIBW.js";
import { a as consumeGatewayFastPathRootOptionToken, n as resolveCliNetworkProxyPolicy, o as consumeGatewayRunOptionToken, t as resolveCliCommandPathPolicy } from "../command-path-policy-Cvup1Nuu.js";
...
async function tryRunGatewayRunFastPath(argv, startupTrace) {
	...
	const [{ Command }, { addGatewayRunCommand }, { VERSION }, { emitCliBanner }, { resolveCliStartupPolicy }] = await startupTrace.measure("gateway-run-imports", () => Promise.all([
		import("commander"),
		import("../run-Bbk9DMem.js"),
		import("../version-CFYXGG_n.js"),
		import("../banner-DIXVV7Km.js"),
		import("../command-startup-policy-85oqTQK1.js")
	]));
...
async function closeCliMemoryManagers() {
	try {
		...
		const { closeActiveMemorySearchManagers } = await import("../memory-runtime-DkT2BEFp.js");
...
async function ensureCliEnvProxyDispatcher() {
	try {
		const { hasEnvHttpProxyAgentConfigured } = await import("../proxy-env-DoliBXIE.js");
		if (!hasEnvHttpProxyAgentConfigured()) return;
		const { ensureGlobalUndiciEnvProxyDispatcher } = await import("../undici-global-dispatcher-8t9L5buR.js");
...
async function bootstrapCliProxyCaptureAndDispatcher(startupTrace) {
	const [{ initializeDebugProxyCapture, finalizeDebugProxyCapture }, { maybeWarnAboutDebugProxyCoverage }] = await startupTrace.measure("proxy-imports", () => Promise.all([import("../runtime-DWpsHyPO.js"), import("../coverage-DKjl3ipe.js")]));
...
async function runCli(argv = process$1.argv) {
	...
	if (shouldLoadCliDotEnv()) await startupTrace.measure("dotenv", async () => {
		const { loadCliDotEnv } = await import("../dotenv-ByjV0GDx.js");
	...
	const stopStartedProxy = async () => {
		...
		proxyHandle = null;
		if (handle) {
			const { stopProxy } = await import("../proxy-lifecycle-DuwVa5Y-.js");
	...
	if (shouldStartProxyForCli(normalizedArgv)) {
		const [{ getRuntimeConfig }, { startProxy }] = await Promise.all([import("../io-DepDp_iZ.js"), import("../proxy-lifecycle-DuwVa5Y-.js")]);
	...
	try {
		if (shouldUseRootHelpFastPath(normalizedArgv)) {
			const { outputPrecomputedRootHelpText } = await import("../root-help-metadata-8bY5OH0-.js");
			if (!outputPrecomputedRootHelpText()) {
				const { outputRootHelp } = await import("../root-help-a3Tt8dBF.js");
				await outputRootHelp();
			}
			return;
		}
		if (shouldUseBrowserHelpFastPath(normalizedArgv)) {
			const { outputPrecomputedBrowserHelpText } = await import("../root-help-metadata-8bY5OH0-.js");
		...
		if (shouldRunBareRootCrestodian) {
			...
			const { createCliProgress } = await import("../progress-C_yzjQiK.js");
		...
		if (bootstrapProxyBeforeFastPath && await tryRunGatewayRunFastPath(normalizedArgv, startupTrace)) return;
		const { tryRouteCli } = await startupTrace.measure("route-import", () => import("../route-DNRliAXb.js"));
		...
		try {
			const { enableConsoleCapture } = await import("../logging-DdL1IaT8.js");
			...
			if (primary && shouldRegisterPrimaryCommandOnly(parseArgv)) await startupTrace.measure("register-primary", async () => {
				const { getProgramContext } = await import("../program-context-JQCWiMSX.js");
				const ctx = getProgramContext(program);
				if (ctx) {
					const { registerCoreCliByName } = await import("../command-registry-BMISLERB.js");
					await registerCoreCliByName(program, ctx, primary, parseArgv);
				}
				const { registerSubCliByName } = await import("../register.subclis-B2_3fuuG.js");
			...
			if (!shouldSkipPluginCommandRegistration({
				...
				if (config) {
					if (primary && !program.commands.some((command) => command.name() === primary || command.aliases().includes(primary))) {
						const { resolveManifestCommandAliasOwner } = await import("../manifest-command-aliases.runtime-C6XgdN3V.js");
