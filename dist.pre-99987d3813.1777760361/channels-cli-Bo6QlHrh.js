...
import { a as normalizeChannelId, i as listChannelPlugins } from "./registry-BD-kXbDr.js";
...
import { n as listBundledPackageChannelMetadata } from "./bundled-package-channel-metadata-iEC1dhfs.js";
import { n as resolveInstallableChannelPlugin } from "./channel-plugin-resolution-DsviN55l.js";
import { t as formatCliChannelOptions } from "./channel-options-CIRltycT.js";
...
function loadChannelsCommands() {
	channelsCommandsPromise ??= import("./channels-Cqf-Lp4l.js");
...
function registerChannelsCli(program) {
	...
	channels.command("list").description("List configured channels + auth profiles").option("--no-usage", "Skip model provider usage/quota snapshots").option("--json", "Output JSON", false).action(async (opts) => {
		await runChannelsCommand(async () => {
			const { channelsListCommand } = await import("./list-CmReCtA6.js");
	...
	channels.command("status").description("Show gateway channel status (use status --deep for local)").option("--probe", "Probe channel credentials", false).option("--timeout <ms>", "Timeout in ms", "10000").option("--json", "Output JSON", false).action(async (opts) => {
		await runChannelsCommand(async () => {
			const { channelsStatusCommand } = await import("./status-D5-L6GwU.js");
