...
import { r as resolveMatrixEnvAuthReadiness } from "./env-auth-Bon-uKig.js";
import { i as resolveMatrixAccount, r as resolveDefaultMatrixAccountId, t as listMatrixAccountIds } from "./accounts-fKbzcSER.js";
import { r as moveSingleMatrixAccountConfigToNamedAccount, t as resolveMatrixSetupDmAllowFrom } from "./setup-dm-policy-1BrSQiN0.js";
...
import { n as validateMatrixHomeserverUrl, r as isPrivateOrLoopbackHost, t as resolveValidatedMatrixHomeserverUrl } from "./url-validation-XjWK_1Jk.js";
import { i as isMatrixSdkAvailable, r as ensureMatrixSdkInstalled } from "./deps-BzNGG-kX.js";
...
async function promptMatrixAllowFrom(params) {
	...
	while (true) {
		...
		if (pending.length > 0) {
			const { resolveMatrixTargets } = await import("./resolve-targets-Cx1igHMZ.js").then((n) => n.n);
	...
	if (accessConfig) if (accessConfig.policy !== "allowlist") next = setMatrixGroupPolicy(next, accessConfig.policy, params.accountId);
		...
		if (accessConfig.entries.length > 0) try {
			...
			for (const entry of accessConfig.entries) {
				...
				const { listMatrixDirectoryGroupsLive } = await import("./directory-live-4oEFsZiz.js").then((n) => n.t);
...
const matrixOnboardingAdapter = {
	...
	afterConfigWritten: async ({ previousCfg, cfg, accountId, runtime }) => {
		const { runMatrixSetupBootstrapAfterConfigWrite } = await import("./setup-bootstrap-Buq0_Uj4.js");
