import{t as n,A as a,m as H,f as r,M as f,q as O,N as U,O as p,o as M,P as D,k as J}from"./index-Zcru02Hy.js";function q(){return[{value:"ok",label:n("cron.runs.runStatusOk")},{value:"error",label:n("cron.runs.runStatusError")},{value:"skipped",label:n("cron.runs.runStatusSkipped")}]}function N(){return[{value:"delivered",label:n("cron.runs.deliveryDelivered")},{value:"not-delivered",label:n("cron.runs.deliveryNotDelivered")},{value:"unknown",label:n("cron.runs.deliveryUnknown")},{value:"not-requested",label:n("cron.runs.deliveryNotRequested")}]}function F(e,t,l){const i=new Set(e);return l?i.add(t):i.delete(t),Array.from(i)}function E(e,t){return e.length===0?t:e.length<=2?e.join(", "):`${e[0]} +${e.length-1}`}function B(e){const t=["last",...e.channels.filter(Boolean)],l=e.form.deliveryChannel?.trim();l&&!t.includes(l)&&t.push(l);const i=new Set;return t.filter(d=>i.has(d)?!1:(i.add(d),!0))}function _(e,t){if(t==="last")return"last";const l=e.channelMeta?.find(i=>i.id===t);return l?.label?l.label:e.channelLabels?.[t]??t}function T(e){return r`
  ...
  </datalist>`}function u(e){return`cron-error-${e}`}function z(e){return e==="name"?"cron-name":e==="scheduleAt"?"cron-schedule-at":e==="everyAmount"?"cron-every-amount":e==="cronExpr"?"cron-cron-expr":e==="staggerAmount"?"cron-stagger-amount":e==="payloadText"?"cron-payload-text":e==="payloadModel"?"cron-payload-model":e==="payloadThinking"?"cron-payload-thinking":e==="timeoutSeconds"?"cron-timeout-seconds":e==="failureAlertAfter"?"cron-failure-alert-after":e==="failureAlertCooldownSeconds"?"cron-failure-alert-cooldown-seconds":"cron-delivery-to"}function Q(e,t,l){return e==="payloadText"?t.payloadKind==="systemEvent"?n("cron.form.mainTimelineMessage"):n("cron.form.assistantTaskPrompt"):e==="deliveryTo"?l==="webhook"?n("cron.form.webhookUrl"):n("cron.form.to"):{name:n("cron.form.fieldName"),scheduleAt:n("cron.form.runAt"),everyAmount:n("cron.form.every"),cronExpr:n("cron.form.expression"),staggerAmount:n("cron.form.staggerWindow"),payloadText:n("cron.form.assistantTaskPrompt"),payloadModel:n("cron.form.model"),payloadThinking:n("cron.form.thinking"),timeoutSeconds:n("cron.form.timeoutSeconds"),deliveryTo:n("cron.form.to"),failureAlertAfter:"Failure alert after",failureAlertCooldownSeconds:"Failure alert cooldown"}[e]}function W(e,t,l){const i=["name","scheduleAt","everyAmount","cronExpr","staggerAmount","payloadText","payloadModel","payloadThinking","timeoutSeconds","deliveryTo","failureAlertAfter","failureAlertCooldownSeconds"],d=[];for(const s of i){const v=e[s];v&&d.push({key:s,label:Q(s,t,l),message:v,inputId:z(s)})}return d}function V(e){const t=document.getElementById(e);t instanceof HTMLElement&&(typeof t.scrollIntoView=="function"&&t.scrollIntoView({block:"center",behavior:"smooth"}),t.focus())}function c(e,t=!1){return r`<span>
    ${e}
    ${t?r`
          <span class="cron-required-marker" aria-hidden="true">*</span>
          <span class="cron-required-sr">${n("cron.form.requiredSr")}</span>
        `:a}
  </span>`}function ae(e){const t=!!e.editingJobId,l=e.form.payloadKind==="agentTurn",i=e.form.scheduleKind==="cron",d=B(e),s=e.runsJobId==null?void 0:e.jobs.find(o=>o.id===e.runsJobId),v=e.runsScope==="all"?n("cron.jobList.allJobs"):s?.name??e.runsJobId??n("cron.jobList.selectJob"),g=e.runs.toSorted((o,h)=>e.runsSortDir==="asc"?o.ts-h.ts:h.ts-o.ts),S=q(),A=N(),$=S.filter(o=>e.runsStatuses.includes(o.value)).map(o=>o.label),I=A.filter(o=>e.runsDeliveryStatuses.includes(o.value)).map(o=>o.label),L=E($,n("cron.runs.allStatuses")),P=E(I,n("cron.runs.allDelivery")),w=e.form.sessionTarget!=="main"&&e.form.payloadKind==="agentTurn",m=e.form.deliveryMode==="announce"&&!w?"none":e.form.deliveryMode,k=W(e.fieldErrors,e.form,m),C=!e.busy&&k.length>0,K=e.jobsQuery.trim().length>0||e.jobsEnabledFilter!=="all"||e.jobsScheduleKindFilter!=="all"||e.jobsLastStatusFilter!=="all"||e.jobsSortBy!=="nextRunAtMs"||e.jobsSortDir!=="asc",j=C&&!e.canSubmit?k.length===1?n("cron.form.fixFields",{count:String(k.length)}):n("cron.form.fixFieldsPlural",{count:String(k.length)}):"";return r`
        ...
        <section class="card">
          ...
          <div class="cron-run-filters">
            ...
            <div class="cron-run-filters__row cron-run-filters__row--secondary">
              ${T({id:"status",title:n("cron.runs.status"),summary:L,options:S,selected:e.runsStatuses,onToggle:(o,h)=>{const x=F(e.runsStatuses,o,h);e.onRunsFiltersChange({cronRunsStatuses:x})},onClear:()=>{e.onRunsFiltersChange({cronRunsStatuses:[]})}})}
              ${T({id:"delivery",title:n("cron.runs.delivery"),summary:P,options:A,selected:e.runsDeliveryStatuses,onToggle:(o,h)=>{const x=F(e.runsDeliveryStatuses,o,h);e.onRunsFiltersChange({cronRunsDeliveryStatuses:x})},onClear:()=>{e.onRunsFiltersChange({cronRunsDeliveryStatuses:[]})}})}
            </div>
          </div>
          ${e.runsScope==="job"&&e.runsJobId==null?r`
              ...
              `:g.length===0?r`
                ...
                `:r`
                  <div class="list" style="margin-top: 12px;">
                    ${g.map(o=>re(o,e.basePath,e.onNavigateToChat))}
                  </div>
                `}
  ...
  `}function Y(e){const t=e.form;return t.scheduleKind==="at"?r`
  ...
  `}function b(e,t){return e?r`<div id=${f(t)} class="cron-help cron-error">${n(e)}</div>`:a}function G(e,t){const i=`list-item list-item-clickable cron-job${t.runsJobId===e.id?" list-item-selected":""}`,d=s=>{t.onLoadRuns(e.id),s()};return r`
  ...
  `}function X(e){if(e.payload.kind==="systemEvent")return r`<div class="cron-job-detail">
    ...
    <div class="cron-job-detail">
      <div class="cron-job-detail-section">
        ...
        <div class="muted cron-job-detail-value chat-text" @click=${Z}>
          ${M(D(e.payload.message))}
        </div>
      </div>
      ${t?r`<div class="cron-job-detail-section">
            <span class="cron-job-detail-label">${n("cron.jobDetail.delivery")}</span>
            <span class="muted cron-job-detail-value">${t.mode}${l}</span>
          </div>`:a}
    </div>
  `}function Z(e){e.target?.closest("a,button,input,textarea,select,summary,[role='button'],[role='link']")&&e.stopPropagation()}function R(e){return typeof e!="number"||!Number.isFinite(e)?n("common.na"):J(e)}function ee(e,t=Date.now()){const l=J(e);return e>t?n("cron.runEntry.next",{rel:l}):n("cron.runEntry.due",{rel:l})}function ne(e){const t=e.state?.lastStatus,l=t==="ok"?"cron-job-status-ok":t==="error"?"cron-job-status-error":t==="skipped"?"cron-job-status-skipped":"cron-job-status-na",i=t==="ok"?n("cron.runs.runStatusOk"):t==="error"?n("cron.runs.runStatusError"):t==="skipped"?n("cron.runs.runStatusSkipped"):n("common.na"),d=e.state?.nextRunAtMs,s=e.state?.lastRunAtMs;return r`
    <div class="cron-job-state">
      ...
      <div class="cron-job-state-row">
        ...
        <span class="cron-job-state-value" title=${p(d)}>
          ${R(d)}
        </span>
      </div>
      <div class="cron-job-state-row">
        <span class="cron-job-state-key">${n("cron.jobState.last")}</span>
        <span class="cron-job-state-value" title=${p(s)}>
          ${R(s)}
        </span>
      </div>
    </div>
  `}function oe(e){switch(e){case"ok":return n("cron.runs.runStatusOk");case"error":return n("cron.runs.runStatusError");case"skipped":return n("cron.runs.runStatusSkipped");default:return n("cron.runs.runStatusUnknown")}}function te(e){switch(e){case"delivered":return n("cron.runs.deliveryDelivered");case"not-delivered":return n("cron.runs.deliveryNotDelivered");case"not-requested":return n("cron.runs.deliveryNotRequested");case"unknown":return n("cron.runs.deliveryUnknown");default:return n("cron.runs.deliveryUnknown")}}function re(e,t,l){const i=typeof e.sessionKey=="string"&&e.sessionKey.trim().length>0?`${U("chat",t)}?session=${encodeURIComponent(e.sessionKey)}`:null,d=oe(e.status??"unknown"),s=te(e.deliveryStatus??"not-requested"),v=e.usage,g=v&&typeof v.total_tokens=="number"?`${v.total_tokens} tokens`:v&&typeof v.input_tokens=="number"&&typeof v.output_tokens=="number"?`${v.input_tokens} in / ${v.output_tokens} out`:null,S=e.summary||e.error||n("cron.runEntry.noSummary"),A=!!e.error&&!!e.summary;return r`
    <div class="list-item cron-run-entry">
      <div class="cron-run-entry__header">
        ...
        <div class="list-meta cron-run-entry__meta">
          <div>${p(e.ts)}</div>
          ${typeof e.runAtMs=="number"?r`<div class="muted">${n("cron.runEntry.runAt")} ${p(e.runAtMs)}</div>`:a}
          <div class="muted">${e.durationMs??0}ms</div>
          ${typeof e.nextRunAtMs=="number"?r`<div class="muted">${ee(e.nextRunAtMs)}</div>`:a}
          ${i?r`<div>
                <a
                  class="session-link"
                  href=${i}
  ...
  `}export{ae as renderCron};
//# sourceMappingURL=cron-BxES8Ten.js.map
