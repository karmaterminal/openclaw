...
import { t as parseLogLine } from "./parse-log-line-DoA9bQrQ.js";
import { a as describeBinding, t as applyAgentBindings } from "./agents.bindings-Dsga2WUw.js";
import { t as requireValidConfigFileSnapshot } from "./config-validation-BLZbRLuE.js";
import { c as requireValidConfig, l as shouldUseWizard, o as channelLabel, s as formatChannelAccountLabel } from "./shared-DnlUPXko.js";
import { t as channelsListCommand } from "./list-WKXNgzu7.js";
import { n as formatGatewayChannelsStatusLines, t as channelsStatusCommand } from "./status-B__3HLOM.js";
...
function loadChannelSetupPluginInstall() {
	channelSetupPluginInstallPromise ??= import("./plugin-install-01-BB_cl.js");
	return channelSetupPluginInstallPromise;
}
function loadOnboardChannels() {
	onboardChannelsPromise ??= import("./onboard-channels-FIFXM7or.js");
...
async function resolveCatalogChannelEntry(raw, cfg) {
	...
	if (!trimmed) return;
	const { listChannelPluginCatalogEntries } = await import("./catalog-gdcoN4Th.js");
...
async function channelsAddCommand(opts, runtime = defaultRuntime, params) {
	...
	if (shouldUseWizard(params)) {
		const [{ buildAgentSummaries }, onboardChannels] = await Promise.all([import("./agents.config-kGWbZD5N.js"), loadOnboardChannels()]);
	...
	if (!channel && catalogEntry) {
		...
		const { isCatalogChannelInstalled } = await import("./discovery-CBIO2aEV.js");
...
async function channelsRemoveCommand(opts, runtime = defaultRuntime, params) {
	...
	const resolvedPluginState = !useWizard && rawChannel && (!channel || !getChannelPlugin(channel)) ? await (async () => {
		const { resolveInstallableChannelPlugin } = await import("./channel-plugin-resolution-DT08wZHM.js");
