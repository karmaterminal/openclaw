...
13. Gate E support-boundary reruns exposed a brittle OpenClaw CLI timeout-wrapper test. The test now lets the fake timeout execute the actual `openclaw` command token and verifies the timeout prefix, command basename, and CLI args, preserving the wrapper contract without depending on one exact absolute argv spelling under broad shard scheduling.
