...
13. Gate E support-boundary reruns exposed a brittle OpenClaw CLI timeout-wrapper test. The test fake now captures the actual `openclaw` command token and verifies the timeout prefix, command basename, and CLI args, preserving the wrapper contract without depending on one exact absolute argv spelling or accidentally executing another OpenClaw binary under broad shard scheduling.
...
- `gates/ronan-openclaw-e2e-instance-after-wrapper-test-fix.log`
- `gates/ronan-support-boundary-after-wrapper-test-fix.log`
- `gates/ronan-openclaw-e2e-instance-after-wrapper-stub-isolation.log`
- `gates/ronan-support-boundary-after-wrapper-stub-isolation.log`
