# WORKORDER — PR #79925 cure-shape (1) — Copilot/gpt-5.5 lane

You are a copilot/gpt-5.5 code-agent lane dispatched by **Silas 🌫** on behalf
...
You are running **in parallel** with a Claude-Opus-4.7 lane firing the same task
spec into a different worktree + candidate ref. Cohort will byte-walk both
authored diffs against `446e285f7d` and against each other for convergence /
divergence. You are not in a race; you are the second harness in a Pattern G
multi-harness rendezvous (per `PRINCE-CODE-AGENT-RUNBOOK.md` § Pattern G).
...
- This worktree: `/home/figs/.openclaw-data/workspace/openclaw-wt-pr79925-cure-1-copilot/`
- **Branch:** `silas/79925-pr-cure-1-copilot-candidate` (off `446e285f7d`)
- **NEVER read, write, list, or shell into:**
  ...
  - `/home/figs/.openclaw-data/workspace/openclaw-wt-pr79925-cure-1-claude/` (Elliott's parallel-lane worktree — do NOT cross-contaminate with reads)
  - `/home/figs/.openclaw/` (live gateway state — runtime)
  - `~/clawd/` (any prince's live runtime)

### §0b — destructive-action scope

- Push to your assigned branch only: `silas/79925-pr-cure-1-copilot-candidate`.
  Push to fork (`origin` = `karmaterminal/openclaw`).
- **Never push to upstream `openclaw/openclaw`.** That is `karmafeast`-PAT
  territory and a separate cohort gate (Cael 🩸 holds the force-push lock for
  the destructive write to `frond-scribe-claude/20260509/narrow-surgery-tight`).
...
- You MAY add the tracking issue to project 56 (or whichever the cohort's
  current SWIM project is — query first, don't guess) with `Status=In Progress`.
...
- **Title:** `[lane] PR #79925 cure-shape (1) — copilot/gpt-5.5 candidate`
- **Body sections** (concrete, fillable):
  ...
  - **Cross-link** to the parallel Claude lane's tracking issue (Elliott 🌻 will
    file his; check the issue list under similar branch-name shape and link)
...
4. **`PRINCE-CODE-AGENT-RUNBOOK.md`** (`/home/figs/.openclaw-data/workspace/openclaw-bootstrap/RUNBOOKS/`) — full document. Pattern A, Pattern E, Pattern G especially. Remote-first push canon, journal-on-branch shape, webhook heartbeat schedule.
...
- **Webhook:** `WEBHOOK_SCRIBE_NOTIFY` from `karmaterminal/silas-likes-to-watch`
- **Resolve:** `gh variable get WEBHOOK_SCRIBE_NOTIFY -R karmaterminal/silas-likes-to-watch`
- **Username override:** `🌫--copilot--🌫` (or `silas/copilot lane` if non-glyph required)
...
## §7 — push canon (remote-first, every meaningful checkpoint)

- Branch already exists remotely (Silas pre-created via `git worktree add -b`).
- After every commit (including the journal-only commits), run:
  `git push origin silas/79925-pr-cure-1-copilot-candidate`
- Never `git push --force` on this branch. Add commits on top.

---

## §8 — declare-done

Final commit must include:

- Tracking issue link in commit body (`Refs: karmaterminal/openclaw#<N>`)
- Brief one-line summary suitable for cohort byte-walk:
  `cure-(1) for #79925 P2: removed inner delivery-time gate, routed both delivery paths through shared resolver, added regression tests, audit in output.md`
- Final webhook post per Checkpoint 6 above.
- Final journal entry with: branch tip SHA, tests-green commands run + output, audit summary, any §9 questions left open.
- **Do NOT open a PR.** This is a candidate ref for cohort byte-walk; Cael 🩸 owns the destructive write to upstream PR #79925 head after cohort cosign.

---

## §9 — open questions / blockers

(Write any blockers, ambiguities, or design questions here in the worktree
journal `tmp-drop-me-pr79925-cure-1.md`. Push the journal commit. Webhook a
"§9 question logged: <one-line>" post.)

---

## §10 — artifact paths (for cohort and figs to inspect mid-run)

- Worktree: `/home/figs/.openclaw-data/workspace/openclaw-wt-pr79925-cure-1-copilot/`
- Workorder (this file): `WORKORDER.md` at worktree root
- Journal: `tmp-drop-me-pr79925-cure-1.md` at worktree root
- Run artifacts: `tmp/codeagents/pr79925-cure-1/copilot-20260514-1423/`
  - `brief.md` — what this run is (a thin pointer to WORKORDER.md)
  - `console.log` — raw harness stdout/stderr
  - `status.txt` — `running` | `complete` | `blocked-§9` | `failed`
  - `pid.txt` — outer process pid
  - `output.md` — final summary: what changed, audit table, tests run, what remains uncertain
- Branch: `silas/79925-pr-cure-1-copilot-candidate` on `karmaterminal/openclaw`

---

🌫 dispatched. take care.
