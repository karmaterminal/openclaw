	...
	if (workspaceDir === sandboxWorkspaceDir) {
		...
		if (cfg.workspaceAccess !== "rw") try {
			const [{ getRemoteSkillEligibility }, { canExecRequestNode }, { syncSkillsToWorkspace }] = await Promise.all([
				import("./skills-remote-CmxwXoUB.js"),
				import("./exec-defaults-Zd0W7yrq.js"),
				import("./skills-Bs3HjHIC.js")
			]);
...
async function resolveSandboxContext(params) {
	...
	if (cfg.prune.idleHours !== 0 || cfg.prune.maxAgeDays !== 0) await (await import("./prune-n-fFftpl.js")).maybePruneSandboxes(cfg);
	...
	const bridgeAuth = cfg.browser.enabled ? await (async () => {
		const cfgForAuth = params.config ?? (await import("./config-FF751HCT.js")).loadConfig();
