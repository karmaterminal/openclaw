...
- Test commands, trusted source: use `pnpm test <path-or-filter> [vitest args...]`, `pnpm test:changed`, `pnpm test:serial`, or `pnpm test:coverage` with scope proportional to the touched contract, and keep local Vitest at one worker (`OPENCLAW_VITEST_MAX_WORKERS=1`). In a worktree, direct local `pnpm test*` is valid when dependencies are ready; use `node scripts/run-vitest.mjs <path-or-filter> --maxWorkers=1` when avoiding pnpm dependency reconciliation is useful. Never raw `vitest`; if unavoidable, `vitest run ...` (bare `vitest` starts watch mode and never exits). No `--repeat`; use a bounded shell loop.
...
- Fork CI acceptance: broad acceptance for `karmaterminal/openclaw` is a bootstrap Mode-B dispatch of `openclaw-local-ci.yml` in `karmaterminal/openclaw-bootstrap` against the exact full 40-character product SHA; record both the product SHA and the run's workflow `headSha`. Mode-B runs hermetic shards on isolated hosted workers and keeps only large/host-bound/dist shards on local seats. `pnpm verify`, untargeted `pnpm test`, `pnpm test:max`, perf full-suite groups, and `scripts/prepush-ci.sh` are upstream-maintainer local kit or exceptional fallbacks, never the ordinary fork landing gate. Commands and two-SHA reporting: `docs/reference/test.md`.
- Proof routing: source trust first, required environment second. Trusted development tests, changed gates, typecheck/lint, and builds run locally with scope proportional to the touched contract; keep local Vitest focused and at one worker, and route broad coverage to Mode-B instead of a shared-tree full suite. Use Crabbox/Testbox only when the environment is part of the proof: clean-machine, install/package, Docker, E2E, live, desktop, cross-OS, CI parity, or explicit operator-requested remote work. Do not use it merely as generic compute offload. Lease/procedure mechanics: `$crabbox`.
...
- Before handoff/push: prove touched surface with focused single-worker runs. Before landing to `main`: proof matches actual risk, and broad proof is a Mode-B run against the exact head SHA, not a local full suite. Bounded behavior-neutral refactor: focused tests/checks enough; no issue proof or broad suite by default.
...
- Vitest rejects Jest `--runInBand`; `OPENCLAW_VITEST_MAX_WORKERS=1` is the standing worker cap for this fork, not just a serial-proof option. Never raise workers on a shared tree to finish faster; the upstream 16-worker ceiling applies only to a dedicated maintainer machine.
