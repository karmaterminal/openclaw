...
- **Multiple rows mature while active:** they aggregate into one bounded provenance note (newest-first, full detail for the most recent few, the remainder summarized by count and flow id), and each row terminalizes after that single durable enqueue — never N notes and never N turns.
