...
- **Note delivery/enqueue fails:** the row is **not** terminalized. Delivery-before-terminalize is the safety invariant — a folded row is consumed only after its trusted note is durably enqueued. On failure the row stays recoverable and retries at a bounded recovery cadence; an "inform and evaporate" that loses the note would be silent loss after the tool already reported the work scheduled.
