...
**Three distinct messages, one easy conflation (the masked `reason`).** A reader who sees an alarming context message must not infer "the session is full and compaction is needed" without a byte-check, because **three architecturally distinct events can surface here, and the most alarming one is decoupled from session fill**:

| #   | Message / event                                                                                      | Source                                                                                   | Means                                                                                                                                                                                                                                                                                                           | Session-fill correlated?                                                                                                 |
| --- | ---------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| 1   | `[system:context-pressure] N% context consumed …`                                                    | `context-pressure.ts` (band check, §4.2)                                                 | **Intentional awareness ping** at a crossed band. Not a failure. Fires through the band cycle: a post-compaction band-0 ping (~10–25%), escalating pings as context rises, then a fresh band-0 ping after the next compaction.                                                                                  | Yes — banded by the session ratio.                                                                                       |
| 2   | `⚠️ Context is too large and auto-compaction could not recover this turn. …`                         | `buildPreflightCompactionFailureText` (`src/auto-reply/reply/agent-runner-execution.ts`) | A **transient turn-level overflow**: one oversized single turn (large inbound payload, an in-context image, a queue-lagged backlog draining at once, or long generated content) that the in-place auto-compactor could not process **for that turn**. The _session_ may be well below the compaction threshold. | **No** — decoupled from session fill. Multi-seat fleet evidence recorded this firing across a 15%–72% spread (see §6.4). |
| 3   | Genuine compaction-failure (the real `compactWithSafetyTimeout` deadline on a very large transcript) | the compaction summary path                                                              | A real compaction that **could not complete** — e.g. a transcript at a large multiple of the context window hitting the summary-call deadline. This is the only one of the three that genuinely means "compaction itself failed."                                                                               | Yes — only fires at genuine high fill.                                                                                   |

The wording of message #2 ("too large … could not recover") reads as message #3, which is why multiple seats misread a benign turn-level transient as a session-capacity failure and nearly elected a needless (and, near the worst case, dangerous) volitional compaction. The discriminator already exists in the runtime but is **masked by default**:

- **The `reason` suffix is verbosity-gated.** `buildPreflightCompactionFailureText` extracts a `reason` from the underlying error but appends it (`… Reason: <reason>.`) **only when `includeDetails` is true** — i.e. only under verbose failure-detail (`isVerboseFailureDetailEnabled`). At default verbosity the discriminating `reason` is suppressed, leaving only the generic alarming sentence. Operators who want the surfaced `reason` on every occurrence should raise the verbose failure-detail level for the agent.
- **The `request_compaction()` guard byte is the ground truth.** Message #2 makes no capacity claim the guard endorses: `request_compaction()` rejects below the threshold with the actual `contextUsage` percentage (e.g. `{status: rejected, guard: context_threshold, contextUsage: 22, threshold: 70}`). A sub-threshold rejection is positive proof you are looking at message #2 (turn-level) or #1 (band ping), **not** message #3.
- **`session_status` shows the real session band.** When in doubt, read the session percentage directly rather than inferring it from the message text.

**Operational rule.** Do **not** announce or elect a volitional `request_compaction()` off message #2 alone. Byte-check first: if `session_status` is below the threshold, or `request_compaction()` rejects sub-threshold, the session is fine — stay up and retry the turn (or trim the oversized inbound). Reserve volitional compaction for a genuine high-band reading (message #1 at a high band, confirmed by `session_status`), where the guard would actually accept. Forcing or electing a needless compaction near the worst case is a real loss of context and, on a seat with a genuinely broken compaction path, risks the very destruction the lifecycle exists to avoid.
