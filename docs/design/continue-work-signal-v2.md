...
**Shipped behavior — finalization anchor and mature-while-active fold (#1135/#1137):** an election captured while its electing turn is still active does not anchor its delay to the tool-call timestamp. The row parks `anchorPending` on that session's end-of-turn lifecycle event; when the turn finalizes the dispatcher stamps `anchorFinalizedAt` and computes the semantic `dueAt = anchorFinalizedAt + delayMs`. (`delaySeconds=0` is therefore "immediately after finalization".) An election captured while the session is already idle anchors immediately to its election time. The slow hedge is a lost-event recovery net only — never the semantic due time.

At the semantic due time the dispatcher makes a closed decision over session state:

- **Due + idle:** grant one same-session continuation turn through `getReplyFromConfig` with a `[continuation:wake]` banner that carries provenance (origin run/turn, elected/anchor/due timeline, delivered-at, overdue-by, chain hop, reason, and disposition). The row terminalizes as `disposition: granted` only after the durable delivered-mark (§3.6 restart-gap cure).
- **Due + active (a later, unrelated turn is running):** do **not** stack a later turn and do **not** re-anchor the delay. Deliver one trusted, high-visible `[system:continuation-note]` into the active session carrying the same provenance, then terminalize the row as `disposition: folded-active`. The note states the disposition explicitly ("folded into this turn; will not fire separately") and strips imperative force: the successor must treat it as prior-turn context and re-evaluate, never as a fresh command.
- **Note delivery/enqueue fails:** the row is **not** terminalized. Delivery-before-terminalize is the safety invariant — a folded row is consumed only after the active turn accepts and commits the trusted note. On failure the row stays recoverable and retries at a bounded recovery cadence; an "inform and evaporate" that loses the note would be silent loss after the tool already reported the work scheduled.
- **Multiple rows mature while active:** they aggregate into one bounded provenance note (newest-first, full detail for the most recent few, the remainder summarized by count and a capped flow-id sample), and each row terminalizes after that single durable enqueue — never N notes and never N turns.

**Provenance is payload, not decoration.** The folded note and granted banner both carry `originRunId`/`originTurnId` (audit identity only), `electedAt`, `anchorFinalizedAt`, semantic `dueAt`, `foldedAt`/`deliveredAt`, `overdueByMs`, `reason`, `chainId`, `hop`, `maxChainLength`, `flowId`, and `disposition`. `originRunId`/`originTurnId` are a separate field from `parentRunId`: they never feed orphan reaping, so same-session origin identity cannot reopen the #952 same-session orphan-reap trap. `reason` is provenance/rate-policy metadata only — it is never an admission gate and never silently drops work (the tool result stays truthful: a row reporting `scheduled` always created durable work).

**Restart recovery (§3.6).** TaskFlow state is authority. On gateway restart, a not-yet-due row re-arms for its semantic due time; an `anchorPending` row whose session is observed idle anchors to the recovery instant (documented fallback for a lost finalization event); a due+idle row grants with provenance; a due+active row folds/informs and terminalizes after durable note delivery; terminal rows take no action. Legacy rows persisted before #1137 lack `anchorFinalizedAt`/`anchorPending` and decode safely onto the pre-#1137 path (no fold, conservative grant-or-busy-skip).
