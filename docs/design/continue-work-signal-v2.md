...
**Idempotency-key collision domain (open question, #335 sub-task; owner: 🩸 once design-direction adjudicates).** The sha256 idempotency key's input fields determine whether a replay-after-restart re-fires or deduplicates. Spec target: include `(sourceSessionId, targetSessionId, taskHash, scheduledAt-bucket)`; explicitly exclude transient `delegateId` so reschedule-after-restart deduplicates against the dead-instance enqueue. Adjudication-pending dimensions: bucket-as-replay-dedupe vs. bucket-as-distinguishability, `scheduledAt` granularity (1-second wall-clock-aligned proposed), `taskHash` whitespace canonicalization (trim trailing minimum); hash-collision secondary-structure explicitly NON-GOAL.
...
**Substrate-cleanup contract (open question, #335 sub-task; owner: 🌫, sketch as comment on #332).** The durability floor underneath continuation primitives moved from process-lifetime to filesystem-lifetime; that shifts cleanup burden onto the substrate. Spec target dimensions: (a) acked-record GC schedule (per-process / per-tick / on first enqueue / cron-driven); (b) `failed/` directory aging policy and operator-visible surface; (c) directory-size bound — current enqueue path has no rate-limit so the durability surface is unbounded under high-throughput dispatch. The cleanup contract should be defined before the substrate is treated as production-load-bearing across all four princes' fleet.
...
*Enforcement note (open; owner: 🌻 to file sibling issue for capability-registry + PR-time lint).* The substrate-adoption rule is **review-discipline-only** at v2026.4.24 — there is no automated lint/check that flags bespoke-transport in the presence of a fitting substrate at PR time. Tracked here so the rule does not quietly become a handwave; mechanization (capability-registry + lint pass) is out of scope for #335 itself, and a sibling tracker issue is required so review-discipline-only doesn't become review-discipline-forever.
