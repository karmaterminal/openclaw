...
**Idempotency-key collision domain (LOCKED 2026-04-26 per design-direction call; implementation owner: 🩸 on canonical2).** The sha256 idempotency key's input fields determine whether a replay-after-restart re-fires or deduplicates. Locked spec: include `(sourceSessionId, targetSessionId, taskHash, scheduledAt-bucket)`; explicitly exclude transient `delegateId` so reschedule-after-restart deduplicates against the dead-instance enqueue. Bucket semantics: **bucket-as-replay-dedupe** (path A); `scheduledAt` is a **1-second wall-clock-aligned bucket**; `taskHash` trims trailing whitespace at minimum. Two legitimate back-to-back identical-tuple enqueues inside the same wall-clock second are the exceptional case and require an explicit caller-supplied `nonce` to opt out of dedupe. Hash-collision secondary-structure (chains/tables/buckets) is explicitly NON-GOAL — the cost is not justified at this surface. The invariant: *same intent replays to the same key unless the caller explicitly asks for multiplicity.*
