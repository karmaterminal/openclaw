...
| `OPENCLAW_GATEWAY_TOKEN`                                                                                                                                                                                                               | Existing Gateway authentication secret. When it contains at least 32 UTF-8 bytes, `diagnostics-otel` also uses it as the domain-separated HMAC key for public-safe run, session, and turn fingerprints. The raw token is never exported. Keep the token fleet-stable for the correlation window; rotating it intentionally breaks joins across the rotation boundary.            |
...
Set the existing `OPENCLAW_GATEWAY_TOKEN` runtime secret to at least 32 UTF-8
bytes on every Gateway process that must join run-owned spans to continuation
spans. OpenClaw domain-separates its telemetry HMAC use from Gateway
authentication and emits these attributes when the source identifiers exist:
