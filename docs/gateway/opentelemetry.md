...
| `OPENCLAW_OTEL_FINGERPRINT_SALT`                                                                                                                                                                                                       | Enables public-safe run, session, and turn correlation on run-owned and continuation spans. Supply one fleet-stable secret of at least 32 UTF-8 bytes through runtime secret injection. OpenClaw uses it only as an HMAC-SHA256 key, exports the first 16 hex characters, and omits the fingerprint attributes when the value is missing or too short. Do not commit the salt or place it in `openclaw.json`; rotating it intentionally breaks joins across the rotation boundary. |
...
### Continuation and proof correlation

Set `OPENCLAW_OTEL_FINGERPRINT_SALT` on every Gateway and one-shot local-run
process that must join run-owned spans to continuation spans. With a valid salt,
OpenClaw emits these HMAC-based attributes when the source identifiers exist:

- `continuation.origin.run.fingerprint`
- `continuation.session.fingerprint`
- `continuation.turn.fingerprint`

The raw run ID, session ID or key, and turn content are never exported. The same
run fingerprint appears on `openclaw.run`, `openclaw.harness.run`,
`openclaw.model.call`, `openclaw.tool.execution`, and the continuation spans
caused by that run. Fingerprints remain stable only while the fleet uses the
same salt.

Accepted continuation entry spans also identify the source without exporting
the prompt or reason:

- `continuation.signal.origin`: `typed-tool`, `tool-call`, `bracket`,
  `post-compaction`, or `queue-drain`
- `continuation.signal.kind`: `work`, `delegate`, or `compaction`
...
Authenticated operator clients can attach a bounded diagnostics context to
`agent`, `chat.send`, or `sessions.send` requests:
...
```json
{
  "diagnosticContext": {
    "proof": {
      ...
      "harnessRef": "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
    }
  }
}
```

`runId` must be 16 lowercase hex characters. `rowId` must be 3 to 64 uppercase
letters, digits, or hyphens and begin with a letter. Both SHA fields must be
full 40-character lowercase Git SHAs. The objects are closed: unknown fields or
malformed values reject the RPC. Valid context is diagnostics-only and does not
grant authorization. OpenClaw carries it across durable continuation and
subagent boundaries and exports only these attributes on the related run,
harness, model-call, tool-execution, continuation, and finalization spans:

- `openclaw.proof.run_id`
- `openclaw.proof.row_id`
- `openclaw.proof.candidate_sha`
- `openclaw.proof.harness_ref`
- `openclaw.proof.synthetic=true`

Traffic without this context remains unmarked. Third-party diagnostic event
producers cannot add proof markers; only context admitted through the trusted
run path is exported.
