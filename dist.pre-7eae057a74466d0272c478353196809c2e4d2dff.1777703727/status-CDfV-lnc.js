...
function formatContinuationBannerValue(params) {
	...
	if (params.pendingDelegatesTotal > 0) parts.push(`${params.pendingDelegatesTotal} delegate${params.pendingDelegatesTotal === 1 ? "" : "s"} pending`);
	if (params.postCompactionStagedTotal > 0) parts.push(`${params.postCompactionStagedTotal} post-compaction`);
...
function loadStatusScanModule() {
	statusScanModulePromise ??= import("./status.scan-DZkp5h9d.js");
	return statusScanModulePromise;
}
function loadStatusScanFastJsonModule() {
	statusScanFastJsonModulePromise ??= import("./status.scan.fast-json-NJRxOcTE.js");
	return statusScanFastJsonModulePromise;
}
function loadStatusAllModule() {
	statusAllModulePromise ??= import("./status-all-Cu82HDNh.js");
	return statusAllModulePromise;
}
function loadStatusCommandTextRuntime() {
	statusCommandTextRuntimePromise ??= import("./status.command.text-runtime-W7cl80GB.js");
	return statusCommandTextRuntimePromise;
}
function loadStatusGatewayConnectionRuntime() {
	statusGatewayConnectionRuntimePromise ??= import("./status.gateway-connection.runtime-Bl6lGAko.js");
	return statusGatewayConnectionRuntimePromise;
}
function loadStatusNodeModeModule() {
	statusNodeModeModulePromise ??= import("./status.node-mode-C94GmbQe.js");
