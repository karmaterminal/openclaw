...
import { n as getChannelsCommandSecretTargetIds } from "./command-secret-targets-BZUQ_E37.js";
import { l as resolveMessageActionDiscoveryForPlugin, r as createMessageActionDiscoveryContext } from "./message-action-discovery-b9yAoI82.js";
import { n as resolveMessageChannelSelection } from "./channel-selection-Br1-uw0d.js";
import { i as resolveChannelDefaultAccountId, r as parseOptionalDelimitedEntries } from "./helpers-nSennZYu.js";
import { c as moveSingleAccountChannelSectionToDefaultAccount } from "./setup-helpers-BccLvWeE.js";
...
import { t as parseLogLine } from "./parse-log-line-BofhtphR.js";
import { a as describeBinding, t as applyAgentBindings } from "./agents.bindings-DYdW_V95.js";
import { t as requireValidConfigFileSnapshot } from "./config-validation-7-gUzCS9.js";
import { c as requireValidConfig, l as shouldUseWizard, o as channelLabel, s as formatChannelAccountLabel } from "./shared-D4s5weqm.js";
import { t as channelsListCommand } from "./list-C6tf6S8O.js";
import { n as formatGatewayChannelsStatusLines, t as channelsStatusCommand } from "./status-BmAAngtQ.js";
...
function loadChannelSetupPluginInstall() {
	channelSetupPluginInstallPromise ??= import("./plugin-install-DIuL79JB.js");
	return channelSetupPluginInstallPromise;
}
function loadOnboardChannels() {
	onboardChannelsPromise ??= import("./onboard-channels-C6I_rG1R.js");
...
async function resolveCatalogChannelEntry(raw, cfg) {
	...
	if (!trimmed) return;
	const { listChannelPluginCatalogEntries } = await import("./catalog-DBZT1cW6.js");
...
async function channelsAddCommand(opts, runtime = defaultRuntime, params) {
	...
	if (shouldUseWizard(params)) {
		const [{ buildAgentSummaries }, onboardChannels] = await Promise.all([import("./agents.config-B2lUMCMd.js"), loadOnboardChannels()]);
	...
	if (!channel && catalogEntry) {
		...
		const { isCatalogChannelInstalled } = await import("./discovery-D9L57xXu.js");
...
async function channelsRemoveCommand(opts, runtime = defaultRuntime, params) {
	...
	const resolvedPluginState = !useWizard && rawChannel && (!channel || !getChannelPlugin(channel)) ? await (async () => {
		const { resolveInstallableChannelPlugin } = await import("./channel-plugin-resolution-9_e244GU.js");
