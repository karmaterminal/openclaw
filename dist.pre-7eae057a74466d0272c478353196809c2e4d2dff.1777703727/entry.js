...
function tryHandleRootVersionFastPath(argv, deps = {}) {
	...
	(deps.resolveVersion ?? (async () => {
		const [{ VERSION }, { resolveCommitHash }] = await Promise.all([import("./version-BAqscPtH.js"), import("./git-commit-N6cvTh4W.js")]);
		return {
			VERSION,
			resolveCommitHash
		};
...
function tryHandleRootHelpFastPath(argv, deps = {}) {
	...
	import("./root-help-metadata-CdmnFuQM.js").then(async ({ outputPrecomputedRootHelpText }) => {
...
function runMainOrRootHelp(argv) {
	...
	import("./run-main-BvarlpqB.js").then(({ runCli }) => runCli(argv)).catch((error) => {
