...
import { n as generateHexPkceVerifierChallenge } from "../../provider-auth--niKqXy8.js";
import "../../error-runtime-Cy7YzM4F.js";
import "../../ssrf-runtime-C0QYuNB-.js";
import "../../gateway-runtime-CBTr9W0R.js";
import { g as resolveRealtimeVoiceAgentConsultToolsAllow, h as resolveRealtimeVoiceAgentConsultTools, m as resolveRealtimeVoiceAgentConsultToolPolicy, s as REALTIME_VOICE_AGENT_CONSULT_TOOL_NAME, t as resolveConfiguredRealtimeVoiceProvider } from "../../provider-resolver-3ymiCPzU.js";
import { o as createRealtimeVoiceBridgeSession, s as consultRealtimeVoiceAgent } from "../../realtime-voice-C4TNdo4f.js";
import { o as waitForLocalOAuthCallback, r as parseOAuthCallbackInput, t as generateOAuthState } from "../../provider-auth-runtime-p7pIUfR3.js";
...
function resolveMeetingInput$1(config, value) {
...
function resolveTokenOptions(config, options) {
	return {
		meeting: resolveMeetingInput$1(config, options.meeting),
...
function registerGoogleMeetCli(params) {
	...
	root.command("join").argument("[url]", "Explicit https://meet.google.com/... URL").option("--transport <transport>", "Transport: chrome, chrome-node, or twilio").option("--mode <mode>", "Mode: realtime for live talk-back, transcribe to join without the realtime voice bridge").option("--message <text>", "Realtime speech to trigger after join").option("--dial-in-number <phone>", "Meet dial-in number for Twilio transport").option("--pin <pin>", "Meet phone PIN; # is appended if omitted").option("--dtmf-sequence <sequence>", "Explicit Twilio DTMF sequence").action(async (url, options) => {
		writeStdoutJson((await (await params.ensureRuntime()).join({
			url: resolveMeetingInput$1(params.config, url),
			transport: options.transport,
			mode: options.mode,
			message: options.message,
			dialInNumber: options.dialInNumber,
...
function readBrowserCreateResult(result) {
	...
	return {
		...
		notes: readStringArray$1(nested.notes),
...
async function createMeetWithBrowserProxyOnNode(params) {
	...
	while (Date.now() <= deadline) try {
		...
		if (result.manualAction) {
			if (result.manualActionReason) throw new Error(`${result.manualActionReason}: ${result.manualAction}`);
...
function normalizeTransport$1(value) {
	return value === "chrome" || value === "chrome-node" || value === "twilio" ? value : void 0;
}
function normalizeMode$1(value) {
...
function hasGoogleMeetOAuth(config, raw) {
	return Boolean(normalizeOptionalString$1(raw.accessToken) ?? normalizeOptionalString$1(raw.refreshToken) ?? config.oauth.accessToken ?? config.oauth.refreshToken);
}
...
async function createAndJoinMeetFromParams(params) {
	...
	const join = await (await params.ensureRuntime()).join({
		...
		transport: normalizeTransport$1(params.raw.transport),
		mode: normalizeMode$1(params.raw.mode),
		dialInNumber: normalizeOptionalString$1(params.raw.dialInNumber),
		pin: normalizeOptionalString$1(params.raw.pin),
		dtmfSequence: normalizeOptionalString$1(params.raw.dtmfSequence),
		message: normalizeOptionalString$1(params.raw.message)
	});
	return {
		...created,
		joined: true,
...
async function openMeetWithBrowserProxy(params) {
	...
	if (params.config.chrome.reuseExistingTab) {
		...
		})).find((entry) => entry.url === params.url);
		targetId = tab?.targetId;
...
var GoogleMeetRuntime = class {
	...
	async join(request) {
		...
		const reusable = this.list().find((session) => session.state === "active" && session.url === url && session.transport === transport && session.mode === mode);
		if (reusable) {
...
const GoogleMeetToolSchema = Type.Object({
	action: Type.String({
		...
		description: "Google Meet action to run. create creates a meeting and joins it by default; pass join=false to only mint a meeting URL."
	}),
...
var google_meet_default = definePluginEntry({
	...
	register(api) {
		...
		api.registerCli(({ program }) => registerGoogleMeetCli({
			program,
			config,
			ensureRuntime
		}), {
