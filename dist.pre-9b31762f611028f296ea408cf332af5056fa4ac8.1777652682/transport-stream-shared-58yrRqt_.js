import { d as resolveDebugProxySettings } from "./runtime-UjYa4Prq.js";
import { a as mergeModelProviderRequestOverrides, c as resolveProviderRequestPolicyConfig, i as getModelProviderRequestTransport, n as buildProviderRequestDispatcherPolicy } from "./provider-request-config-CVScCNkv.js";
...
function transformTransportMessages(messages, model, normalizeToolCallId) {
	...
	for (const msg of transformed) {
		if (msg.role === "assistant") {
			if (allowSyntheticToolResults && pendingToolCalls.length > 0) appendMissingToolResults(result, pendingToolCalls, existingToolResultIds);
			pendingToolCalls = [];
