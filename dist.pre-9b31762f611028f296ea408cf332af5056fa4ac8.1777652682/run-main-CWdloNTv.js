...
import { F as getPositiveIntFlagValue, G as isValueToken, L as getVerboseFlag, N as getCommandPositionalsWithRootOptions, P as getFlagValue, R as hasFlag } from "./logger--l1ykZMf.js";
import { t as resolveCliArgvInvocation } from "./argv-invocation-WovolAlT.js";
import { a as parseCliContainerArgs, i as maybeRunCliInContainer, n as applyCliProfileEnv, r as parseCliProfileArgs, t as normalizeWindowsArgv } from "./windows-argv-DzpibptW.js";
import { n as defaultRuntime } from "./runtime-CHryl7ev.js";
import { a as enableConsoleCapture } from "./subsystem-DRUx3zf3.js";
import { i as normalizeEnv, t as isTruthyEnvValue } from "./env-DM4mWVZJ.js";
import { _ as resolveStateDir } from "./paths-CrDgDBYA.js";
import { t as assertSupportedRuntime } from "./runtime-guard-BUBdDBtJ.js";
import { t as resolveManifestCommandAliasOwner } from "./manifest-command-aliases.runtime-BDu7GJQC.js";
import { s as hasMemoryRuntime } from "./memory-state-SVbJI-Hg.js";
import "./logging-Cnra-2pM.js";
import { i as initializeDebugProxyCapture, r as finalizeDebugProxyCapture } from "./runtime-UjYa4Prq.js";
...
import { i as shouldSkipPluginCommandRegistration, n as shouldRegisterPrimaryCommandOnly } from "./command-registration-policy-ByQdxyCB.js";
import { a as shouldEnsureCliPathForCommandPath, c as cliCommandCatalog, n as ensureCliExecutionBootstrap, o as resolveCliCommandPathPolicy, r as resolveCliExecutionStartupContext, s as matchesCommandPath, t as applyCliExecutionStartupPresentation } from "./command-execution-startup-Cz9tXooK.js";
import { n as maybeWarnAboutDebugProxyCoverage } from "./coverage-DCXuwxbS.js";
...
function loadConfigCli() {
	configCliPromise ??= import("./config-cli-CRHBRVtF.js");
	return configCliPromise;
}
function loadModelsList() {
	modelsListPromise ??= import("./list-DZ6Bx-ww.js");
...
const routedCommandDefinitions = {
	health: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { healthCommand } = await import("./health-CIxZnWLp.js");
	...
	status: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			if (args.json) {
				const { statusJsonCommand } = await import("./status-json-BE7CA7dZ.js");
			...
			const { statusCommand } = await import("./status-DoYB-xjY.js");
	...
	"gateway-status": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { runDaemonStatus } = await import("./status-CRh8CnBR.js");
	...
	sessions: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { sessionsCommand } = await import("./sessions-Ba6wwn5G.js");
	...
	"agents-list": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { agentsListCommand } = await import("./agents-CdX5gCrB.js");
	...
	"channels-list": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { channelsListCommand } = await import("./list-I9MCO4Ln.js");
	...
	"channels-status": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { channelsStatusCommand } = await import("./status-CPVrFxFZ.js");
...
async function prepareRoutedCommand(params) {
	...
	const { VERSION } = await import("./version-BAqscPtH.js");
...
async function closeCliMemoryManagers() {
	...
	try {
		const { closeActiveMemorySearchManagers } = await import("./memory-runtime-AAlgPdFv.js");
...
async function runCli(argv = process$1.argv) {
	...
	if (shouldLoadCliDotEnv()) {
		const { loadCliDotEnv } = await import("./dotenv-D5TdXDTP.js");
	...
	try {
		if (shouldUseRootHelpFastPath(normalizedArgv)) {
			const { outputPrecomputedRootHelpText } = await import("./root-help-metadata-Cz6B39mA.js");
			if (!outputPrecomputedRootHelpText()) {
				const { outputRootHelp } = await import("./root-help-DrSat590.js");
		...
		if (primary && shouldRegisterPrimaryCommandOnly(parseArgv)) {
			const { getProgramContext } = await import("./program-context-DFhYAkNm.js");
			const ctx = getProgramContext(program);
			if (ctx) {
				const { registerCoreCliByName } = await import("./command-registry-BnbzTAht.js");
				await registerCoreCliByName(program, ctx, primary, parseArgv);
			}
			const { registerSubCliByName } = await import("./register.subclis-CpE_zFq9.js");
