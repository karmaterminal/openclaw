...
const decoders = {
	async oga(buf) {
		...
		if (!decoder) {
			let { OggVorbisDecoder } = await import("./ogg-vorbis-B7biPLBg.js");
	...
	async mp3(buf) {
		...
		if (!decoder) {
			const { MPEGDecoder } = await import("./mpg123-decoder-CH0UWFfB.js");
	...
	async flac(buf) {
		...
		if (!decoder) {
			const { FLACDecoder } = await import("./flac-BQiruysW.js");
	...
	async opus(buf) {
		...
		if (!decoder) {
			const { OggOpusDecoder } = await import("./ogg-opus-decoder-C4yldHA8.js");
	...
	async wav(buf) {
		...
		if (!decode) {
			let module = await import("./node-wav-DY8wjDVt.js").then((m) => /* @__PURE__ */ __toESM(m.default, 1));
	...
	async qoa(buf) {
		...
		if (!decode) {decoders.qoa.decode = decode = (await import("./qoa-format-Df6N_tJP.js")).decode;}
		return buf && createBuffer(await decode(buf));
	}
};
