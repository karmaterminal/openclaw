...
import { t as require_dist_cjs$1 } from "./dist-cjs-oujylQVy.js";
...
var require_dist_cjs = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	const profileNameBlockList = new Set(["__proto__", "profile __proto__"]);
	const parseIni = (iniData) => {
		...
		for (const iniLine of iniData.split(/\r?\n/)) {
			...
			if (trimmedLine[0] === "[" && trimmedLine[trimmedLine.length - 1] === "]") {
				...
				if (profileNameBlockList.has(sectionName)) {throw new Error(`Found invalid profile name "${sectionName}"`);}
