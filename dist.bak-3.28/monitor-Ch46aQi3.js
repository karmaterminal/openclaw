...
import { $n as resolveFeishuReplyPolicy, An as sendMessageFeishu, Cr as createFeishuWSClient, Dn as resolveFeishuCardTemplate, Dr as resolveFeishuRuntimeAccount, En as listFeishuThreadMessages, Er as resolveFeishuAccount, Fn as buildMentionedCardContent, Ln as extractMentionTargets, Nn as parsePostContent, On as sendCardFeishu, PP as buildAgentMediaPayload, Pn as normalizeFeishuExternalKey, Qn as resolveFeishuGroupConfig, Sr as createFeishuClient, Tn as getMessageFeishu, Tr as listEnabledFeishuAccounts, Un as isMentionForwardRequest, Wn as getFeishuRuntime, Xn as isFeishuGroupAllowed, Zn as resolveFeishuAllowlistMatch, _r as buildFeishuConversationId, ar as decodeFeishuCardAction, fr as probeFeishu, ir as createFeishuCardInteractionEnvelope, jn as sendStructuredCardFeishu, lP as getSessionBindingService, lr as createFeishuThreadBindingManager, mC as resolveConfiguredBindingRoute, mr as waitForAbortableDelay, nr as resolveReceiveIdType, or as createPersistentDedupe, pC as ensureConfiguredBindingRouteReady, pr as raceWithTimeoutAndAbort, rr as buildFeishuCardActionTextFallback, vn as downloadMessageResourceFeishu, wr as require_lib, xn as sendMediaFeishu, xr as createEventDispatcher } from "./auth-profiles-D5vQ2NEm.js";
import { c as normalizeAgentId, u as resolveAgentIdFromSessionKey } from "./session-key-BhxcMJEE.js";
import { r as deriveLastRoutePolicy } from "./base-session-key-UJINc15Z.js";
...
import { h as sendMediaWithLeadingCaption, m as resolveTextChunksWithFallback, p as resolveSendableOutboundReplyParts } from "./reply-payload-CJqP_sJ6.js";
import { a as createReplyPrefixContext, t as createChannelReplyPipeline } from "./channel-reply-pipeline-DFacxqeY.js";
import { a as isRequestBodyLimitError, c as requestBodyErrorToText, i as installRequestBodyLimitGuard, s as readRequestBodyWithLimit } from "./http-body-JDY8COhf.js";
import { r as applyBasicWebhookRequestGuards } from "./webhook-request-guards-BcAHiveC.js";
import { a as buildPendingHistoryContextFromMap, s as clearHistoryEntriesIfEnabled, u as recordPendingHistoryEntryIfEnabled } from "./reply-history-Zf0VECih.js";
	...
	try {
		...
		const resolveThreadContextForAgent = async (agentId, agentSessionKey) => {
			...
			try {
				...
				const historyMessages = rootMsg?.content || ctx.rootId ? relevantMessages : relevantMessages.slice(1);
	...
	server.on("request", (req, res) => {
		...
		(async () => {
			try {
				...
				const { isChallenge, challenge } = import_lib.generateChallenge(payload, { encryptKey: account.encryptKey ?? "" });
				if (isChallenge) {
					res.statusCode = 200;
