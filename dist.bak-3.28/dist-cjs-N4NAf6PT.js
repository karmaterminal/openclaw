...
import { t as require_dist_cjs$11 } from "./dist-cjs-oujylQVy.js";
import { t as require_dist_cjs$12 } from "./dist-cjs-CBMN2jq5.js";
import { n as require_dist_cjs$13, t as require_dist_cjs$14 } from "./dist-cjs-CUIiZzMm.js";
import { t as require_dist_cjs$15 } from "./dist-cjs-DUdQ9aik.js";
...
var require_dist_cjs$7 = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function buildQueryString(query) {
		const parts = [];
		for (let key of Object.keys(query).toSorted()) {
...
var require_dist_cjs$6 = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	const NODEJS_TIMEOUT_ERROR_CODES = new Set([
		"ECONNRESET",
		"EPIPE",
		"ETIMEDOUT"
	]);
...
or increase socketAcquisitionWarningTimeout=(millis) in the NodeHttpHandler config.`);
		...
		async handle(request, { abortSignal, requestTimeout } = {}) {
			...
			return new Promise((_resolve, _reject) => {
				...
				req.on("error", (err) => {
					if (NODEJS_TIMEOUT_ERROR_CODES.has(err.code)) {reject(Object.assign(err, { name: "TimeoutError" }));}
					else {reject(err);}
				});
				if (abortSignal) {
					const onAbort = () => {
...
var init_HttpProtocol = __esmMin((() => {
	...
	HttpProtocol = class extends SerdeContext {
		...
		async loadEventStreamCapability() {
			const { EventStreamSerde } = await import("./event-streams-D1-XsG97.js");
...
var require_dist_cjs$1 = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	const constructStack = () => {
		...
		const expandRelativeMiddlewareList = (from) => {
			...
			from.after.toReversed().forEach((entry) => {
				if (entry.before.length === 0 && entry.after.length === 0) {expandedMiddlewareList.push(entry);}
				else {expandedMiddlewareList.push(...expandRelativeMiddlewareList(entry));}
			});
		...
		const stack = {
			...
			resolve: (handler, context) => {
				for (const middleware of getMiddlewareList().map((entry) => entry.middleware).toReversed()) {handler = middleware(handler, context);}
