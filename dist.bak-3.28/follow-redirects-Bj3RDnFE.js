...
var require_follow_redirects = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	RedirectableRequest.prototype._performRequest = function() {
		...
		this._currentUrl = this._options.path.startsWith('/') ? url.format(this._options) : this._options.path;
		if (this._isRedirect) {
			var i = 0;
			var self = this;
	...
	function validateUrl(input) {
		if (input.hostname.startsWith('[') && !/^\[[:0-9a-f]+\]$/i.test(input.hostname)) {throw new InvalidUrlError({ input: input.href || input });}
		if (input.host.startsWith('[') && !/^\[[:0-9a-f]+\](:\d+)?$/i.test(input.host)) {throw new InvalidUrlError({ input: input.href || input });}
		return input;
	}
