...
import { g as readConfigFileSnapshot, x as writeConfigFile } from "./auth-profiles-D5vQ2NEm.js";
import { u as resolveGatewayPort } from "./paths-Y4UT24Of.js";
import { l as normalizeSecretInputString } from "./types.secrets-Rqz2qv-w.js";
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const { resolvePluginProviders, resolveProviderPluginChoice } = await import("./provider-auth-choice.runtime-BN3zylPJ.js");
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-w5jLeS_P.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-J9gBh4R2.js");
		const { logConfigUpdated } = await import("./logging-wrxybgSp.js");
	...
	const { applyLocalSetupWorkspaceConfig } = await import("./onboard-config-Ljc25l71.js");
	let nextConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
	const { ensureAuthProfileStore } = await import("./agents/auth-profiles.runtime.js");
	const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-Cd1JojPQ.js");
	const { promptCustomApiConfig } = await import("./onboard-custom-Ctk2FXSa.js");
	const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-BLIxjQkq.js");
	const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-hueSFLbX.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-DoD0TS6b.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) {await prompter.note("Skipping channel setup.", "Channels");}
		...
		const { listChannelPlugins } = await import("./plugins-Du9ips4j.js");
		const { setupChannels } = await import("./onboard-channels-0PE2x5nd.js");
	...
	if (opts.skipSearch) {await prompter.note("Skipping search setup.", "Search");}
		...
		const { setupSearch } = await import("./onboard-search-10YDKOcj.js");
	...
	if (opts.skipSkills) {await prompter.note("Skipping skills setup.", "Skills");}
		...
		const { setupSkills } = await import("./onboard-skills-VNSNQRVY.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	const { setupInternalHooks } = await import("./onboard-hooks-C25YjCdi.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-CtCQvw_q.js");
