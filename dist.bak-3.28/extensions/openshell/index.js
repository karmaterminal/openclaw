...
import { $T as runPluginCommandWithTimeout, D_ as registerSandboxBackend, F_ as runSshSandboxCommand, I_ as shellEscape, M_ as createSshSandboxSessionFromConfigText, P_ as disposeSshSandboxSession, R_ as createRemoteShellSandboxFsBridge, k_ as buildExecRemoteCommand, z_ as createWritableRenameTargetResolver } from "../../auth-profiles-D5vQ2NEm.js";
...
async function replaceDirectoryContents(params) {
	...
	const isExcluded = (name) => excluded.has(name.toLowerCase());
	await fs.mkdir(params.targetDir, { recursive: true });
	const existing = await fs.readdir(params.targetDir);
	await Promise.all(existing.filter((entry) => !isExcluded(entry)).map((entry) => fs.rm(path.join(params.targetDir, entry), {
		recursive: true,
		force: true
	})));
