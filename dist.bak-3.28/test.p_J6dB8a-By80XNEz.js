...
function getKeysOfEnumerableProperties(object, compareKeys) {
	...
	const keys = compareKeys === null ? rawKeys : rawKeys.toSorted(compareKeys);
...
function serializeDOM(node, config, indentation, depth, refs, printer, filterNode) {
	...
	return printElement(type, printProps(nodeIsFragment(node) ? [] : Array.from(node.attributes, (attr) => attr.name).toSorted(), nodeIsFragment(node) ? {} : [...node.attributes].reduce((props, attribute) => {
		props[attribute.name] = attribute.value;
		return props;
...
function sortMap(map) {
	return new Map(Array.from(map.entries()).toSorted());
}
function sortSet(set) {
	return new Set(Array.from(set.values()).toSorted());
}
...
function createSuiteCollector(name, factory = () => {}, mode, each, suiteOptions) {
	...
	const task = function(name = "", options = {}) {
		...
		}).filter((r) => r != null).toSorted((tag1, tag2) => (tag2.priority ?? POSITIVE_INFINITY) - (tag1.priority ?? POSITIVE_INFINITY)).reduce((acc, tag) => {
...
function entriesEqual(leftHandOperand, rightHandOperand, options) {
	...
	return iterableEqual(leftHandItems.toSorted(), rightHandItems.toSorted(), options);
}
...
function objectEqual(leftHandOperand, rightHandOperand, options) {
	...
	if (leftHandKeys.length && leftHandKeys.length === rightHandKeys.length) {
		if (!iterableEqual(mapSymbols(leftHandKeys).toSorted(), mapSymbols(rightHandKeys).toSorted())) {return false;}
...
function assertKeys(keys) {
	...
	this.assert(ok, "expected #{this} to " + deepStr + str, "expected #{this} to not " + deepStr + str, expected.slice(0).toSorted(compareByInspect), actual.toSorted(compareByInspect), true);
}
...
async function saveInlineSnapshots(environment, snapshots) {
	const MagicString = (await import("./magic-string.es-7cxPL6N4.js")).default;
...
async function saveSnapshotFile(environment, snapshotData, snapshotPath) {
	const snapshots = Object.keys(snapshotData).toSorted(naturalCompare).map((key) => `exports[${printBacktickString(key)}] = ${printBacktickString(normalizeNewlines(snapshotData[key]))};`);
...
function createExpectPoll(expect) {
	return function poll(fn, options = {}) {
		...
		const proxy = new Proxy(assertion, { get(target, key, receiver) {
			...
			if (typeof key === "string" && unsupported.has(key)) {throw new SyntaxError(`expect.poll() is not supported in combination with .${key}(). Use vi.waitFor() if your assertion condition is unstable.`);}
			return function(...args) {
		...
		*/
		function install(config) {
			...
			*/
			...
			if (config.shouldAdvanceTime) {clock.setTickMode({
				mode: "interval",
...
function createVitest() {
	...
	const utils = {
		...
		stubEnv(name, value) {
			...
			if (_envBooleans.has(name)) {env[name] = value ? "1" : "";}
...
//#endregion
export { vi as n, it as r, globalExpect as t };
