...
import { c as highestDelayedContinuationReservationHop, i as consumePendingDelegates, l as peekSoonestUnmaturedDelegateDueAt } from "./delegate-store-BKkoivDc.js";
...
import { n as resolveContinuationRuntimeConfig } from "./config-xFdjNQEi.js";
import { l as retainContinuationTimerRef, o as registerContinuationTimerHandle, u as unregisterContinuationTimerHandle } from "./state-etYw3MLs.js";
...
function armHedgeTimer(sessionKey, fireAt, params) {
	...
	const handle = setTimeout(() => {
		...
		dispatchToolDelegates({
			...
			log.warn(`[continuation:delegate-hedge-error] error=${err instanceof Error ? err.message : String(err)} session=${sessionKey}`);
		});
	}, fireIn);
...
async function dispatchToolDelegates(params) {
	...
	for (const dropped of delegatesOverLimit) {
		...
		enqueueSystemEvent(`[continuation] Tool delegate rejected: maxDelegatesPerTurn exceeded (${maxDelegatesPerTurn}). Task: ${dropped.task}`, { sessionKey });
	}
	let dispatched = 0;
	...
	for (const delegate of delegatesWithinLimit) {
		...
		if (budgetCheck) {
			...
			enqueueSystemEvent(`[continuation] Tool delegate rejected: ${budgetCheck}. Task: ${delegate.task}`, { sessionKey });
			rejected++;
			continue;
		}
		...
		try {
			...
			if (result.status === "accepted") {
				...
				enqueueSystemEvent(`[continuation] DELEGATE spawn ${result.status}: delegation was not accepted. Task: ${delegate.task}`, { sessionKey });
				rejected++;
			}
		} catch (err) {
			log.info(`[continuation:delegate-spawn-failed] error=${err instanceof Error ? err.message : String(err)} session=${sessionKey}`);
			enqueueSystemEvent(`[continuation] DELEGATE spawn failed: ${String(err)}. Task: ${delegate.task}`, { sessionKey });
			rejected++;
		}
	}
	return {
...
*/
async function dispatchPostCompactionDelegates(delegates, sessionKey, spawnCtx) {
	let dispatched = 0;
	let failed = 0;
...
//#endregion
export { dispatchToolDelegates as n, resetDelegateDispatchHedgesForTests as r, dispatchPostCompactionDelegates as t };
