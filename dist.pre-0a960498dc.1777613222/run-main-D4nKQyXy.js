...
function loadConfigCli() {
	configCliPromise ??= import("./config-cli-CuJMCfTO.js");
	return configCliPromise;
}
function loadModelsList() {
	modelsListPromise ??= import("./list-k8oaFtEI.js");
...
const routedCommandDefinitions = {
	health: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { healthCommand } = await import("./health-7VoLYUAI.js");
	...
	status: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			if (args.json) {
				const { statusJsonCommand } = await import("./status-json-CSHEdtBk.js");
			...
			const { statusCommand } = await import("./status-DsFdu4Z_.js");
	...
	"gateway-status": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { runDaemonStatus } = await import("./status-aqkOjbiM.js");
	...
	"agents-list": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { agentsListCommand } = await import("./agents-BLisuPO-.js");
	...
	"channels-list": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { channelsListCommand } = await import("./list-Bng0augc.js");
	...
	"channels-status": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { channelsStatusCommand } = await import("./status-DTC_BSEg.js");
...
async function runCli(argv = process$1.argv) {
	...
	try {
		...
		if (primary && shouldRegisterPrimaryCommandOnly(parseArgv)) {
			...
			if (ctx) {
				const { registerCoreCliByName } = await import("./command-registry-BE2nyeHu.js");
				await registerCoreCliByName(program, ctx, primary, parseArgv);
			}
			const { registerSubCliByName } = await import("./register.subclis-H5XKO2V6.js");
