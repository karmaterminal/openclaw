...
import { E as estimateMessagesTokens, k as readPostCompactionContext } from "./wait-for-idle-before-flush-DV1kMVgL.js";
import { a as isAudioFileName } from "./mime-CeX1JZPE.js";
import { l as isLikelyExecutionAckPrompt } from "./selection-sNhZ1IUb.js";
...
import { u as resolveContinuationRuntimeConfig } from "./subagent-announce-delivery-wvPDeTRD.js";
...
import { n as spawnSubagentDirect } from "./subagent-spawn-BM1Innks.js";
import "./sandbox-D2A6r4WD.js";
import { t as runEmbeddedPiAgent } from "./pi-embedded-vqfGRXoB.js";
import { n as buildAgentRuntimeOutcomePlan, t as buildAgentRuntimeDeliveryPlan } from "./build-C3n5eUP9.js";
...
import { a as resolveModelFallbackOptions, c as resolveRunAuthProfile, i as isBunFetchSocketError, o as resolveQueuedReplyExecutionConfig, r as formatBunFetchSocketError, s as resolveQueuedReplyRuntimeConfig, t as buildEmbeddedRunExecutionParams } from "./agent-runner-utils-BIZNMNXG.js";
...
async function runEmbeddedPiAgentDefault$1(...args) {
	const { runEmbeddedPiAgent } = await import("./pi-embedded.runtime-DW-TTRnv.js");
						...
						const result = await runEmbeddedPiAgentDefault$1({
							...
							requestCompactionOpts: runtimeConfig?.agents?.defaults?.continuation?.enabled === true ? {
								...
								triggerCompaction: async () => {
									try {
										const { compactEmbeddedPiSession } = await import("./compact.queued-DeTtyjlT.js");
...
function loadPiEmbeddedRuntime() {
	piEmbeddedRuntimePromise ??= import("./pi-embedded-D7zEvTI0.js");
		...
		try {
			...
			if (runtimeConfig?.agents?.defaults?.continuation?.enabled === true && sessionKey) {
				const [{ dispatchToolDelegates }, { resolveContinuationRuntimeConfig }, { loadContinuationChainState, persistContinuationChainState }, { updateSessionStore, resolveSessionStoreEntry }] = await Promise.all([
					import("./delegate-dispatch-BJe5rzlL.js"),
...
async function runReplyAgent(params) {
	...
	try {
		...
		if (cfg?.agents?.defaults?.continuation?.enabled === true && sessionKey && activeSessionEntry) {
			const { checkContextPressure } = await import("./lazy.runtime-ybjk9EBG.js");
		...
		if (continuationFeatureEnabled && sessionKey) {
			const turnTokens = (usage?.input ?? 0) + (usage?.output ?? 0);
			const { dispatchToolDelegates, loadContinuationChainState } = await import("./lazy.runtime-ybjk9EBG.js");
		...
		if ((effectiveContinuationSignal || hasQueuedDelegateWork) && sessionKey && activeSessionEntry) {
			const { loadContinuationChainState } = await import("./lazy.runtime-ybjk9EBG.js");
			const turnTokens = (usage?.input ?? 0) + (usage?.output ?? 0);
