...
import { a as isSilentReplyText, c as stripLeadingSilentToken, l as stripSilentToken, n as SILENT_REPLY_TOKEN, o as startsWithSilentToken, s as stripContinuationSignal } from "./tokens-2Eginc2V.js";
import { r as callGateway } from "./call-O_DAf3Ts.js";
import "./delivery-context-D9Edfyha.js";
import { i as consumePendingDelegates } from "./delegate-store-BKkoivDc.js";
import { a as resolveSubagentCompletionOrigin, c as buildAnnounceIdFromChildRun, i as resolveSubagentAnnounceTimeoutMs, l as buildAnnounceIdempotencyKey, n as loadRequesterSessionEntry, o as runAnnounceDeliveryWithRetry, r as loadSessionEntryByKey, s as resolveAnnounceOrigin, t as deliverSubagentAnnouncement, u as resolveContinuationRuntimeConfig } from "./subagent-announce-delivery-wvPDeTRD.js";
import { t as getSubagentDepthFromSessionStore } from "./subagent-depth-D5Jjo3hX.js";
import { t as formatAgentInternalEventsForPrompt } from "./internal-events-DZStiG-Y.js";
import { a as isEmbeddedPiRunActive, f as waitForEmbeddedPiRunEnd } from "./runs-rRV0MELk.js";
import { a as dedupeLatestChildCompletionRows, c as readSubagentOutput, d as importRuntimeModule, h as isAnnounceSkip, i as captureSubagentCompletionReply, l as waitForSubagentRunOutcome, n as buildChildCompletionFindings, o as filterCurrentDirectChildCompletionRows, r as buildCompactAnnounceStatsLine, s as readLatestSubagentOutputWithRetry, t as applySubagentWaitOutcome } from "./subagent-announce-output-DIidB__j.js";
import { t as buildSubagentSystemPrompt } from "./subagent-system-prompt-Cv7t3Jhv.js";
...
function loadSubagentRegistryRuntime() {
	subagentRegistryRuntimePromise ??= import("./subagent-announce.registry.runtime-Cz1FgcJh.js");
	return subagentRegistryRuntimePromise;
}
function loadContinuationStateRuntime() {
	continuationStateRuntimePromise ??= import("./continuation-state.runtime-CQHcePnT.js");
	return continuationStateRuntimePromise;
}
function loadSubagentSpawnRuntime() {
	subagentSpawnRuntimePromise ??= import("./subagent-spawn-BRd5x1Q3.js");
	...
	try {
		...
		if (continuationEnabled && (findings !== "(no output)" || toolDelegates.length > 0)) {
			...
			if (continuationResult.signal?.kind === "work") defaultRuntime.log(`[subagent-chain-hop] CONTINUE_WORK not supported in sub-agent chain (from ${params.childSessionKey}), ignoring`);
				...
				const chainSilent = continuationResult.signal.silent || continuationResult.signal.silentWake || parentWasSilent;
				const chainWake = continuationResult.signal.silentWake || parentWasSilent && params.wakeOnReturn === true;
				const { maxChainLength, costCapTokens, minDelayMs, maxDelayMs } = resolveContinuationRuntimeConfig(cfg);
			...
			if (toolDelegates.length > 0 && isContinuationChainDelegate) {
				const { maxChainLength: toolMaxChainLength, costCapTokens: toolCostCapTokens, minDelayMs: toolMinDelayMs, maxDelayMs: toolMaxDelayMs } = resolveContinuationRuntimeConfig(cfg);
				const hopMatch = childTask.match(/\[continuation:chain-hop:(\d+)\]/);
		...
		if (params.silentAnnounce) {
			const { enqueueSystemEvent } = await import("./system-events-DbQadb-F.js");
			const { createSubsystemLogger } = await import("./subsystem-DGdJpCcI.js");
			...
			enqueueSystemEvent(triggerMessage || `[continuation:enrichment-return] Delegate completed: ${taskLabel}`, { sessionKey: targetRequesterSessionKey });
			...
			if (params.wakeOnReturn) {
				const { requestHeartbeatNow } = await import("./heartbeat-wake-DNmYUtWi.js");
				requestHeartbeatNow({
