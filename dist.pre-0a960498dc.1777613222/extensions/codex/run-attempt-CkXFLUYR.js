...
function sanitizePermissionPathValue(value) {
	return truncate(sanitizePermissionScalar(value).replace(/^\/home\/(?!\.{1,2}(?=\/|$))[^/]+(?=\/|$)/, "~").replace(/^\/Users\/(?!\.{1,2}(?=\/|$))[^/]+(?=\/|$)/, "~").replace(/^[A-Za-z]:[\\/]Users[\\/](?!\.{1,2}(?=[\\/]|$))[^\\/]+(?=[\\/]|$)/i, "~"), PERMISSION_VALUE_MAX_LENGTH);
}
...
function readBridgeableApprovalElicitation(requestParams) {
	...
	const title = readString$3(requestParams, "message") ?? "Codex MCP tool approval";
	return {
		title,
...
function buildApprovalDescription(params) {
	const summaryLines = [
		readString$3(params.meta, MCP_TOOL_APPROVAL_CONNECTOR_NAME_KEY) && `App: ${readString$3(params.meta, MCP_TOOL_APPROVAL_CONNECTOR_NAME_KEY)}`,
		readString$3(params.meta, MCP_TOOL_APPROVAL_TOOL_TITLE_KEY) && `Tool: ${readString$3(params.meta, MCP_TOOL_APPROVAL_TOOL_TITLE_KEY)}`,
...
function readPropertyDescriptionLines(requestedSchema) {
	...
	return Object.entries(properties).map(([name, value]) => {
		...
		if (!schema) return;
		const propTitle = readString$3(schema, "title") ?? name;
		const description = readString$3(schema, "description");
...
function readDisplayParamLines(meta) {
	...
	return displayParams.map((entry) => {
		...
		if (!param) return;
		const name = readString$3(param, "display_name") ?? readString$3(param, "name");
		if (!name) return;
