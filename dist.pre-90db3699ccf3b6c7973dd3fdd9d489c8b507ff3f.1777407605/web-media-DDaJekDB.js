import { r as lowercasePreservingWhitespace } from "./string-coerce-CjxCKZ6B.js";
...
import { c as mimeTypeFromFilePath, l as normalizeMimeType, m as maxBytesForKind, n as extensionForMime, r as getFileExtension, s as kindFromMime, t as detectMime } from "./mime-Dx8S3QoZ.js";
import { c as optimizeImageToPng, i as convertHeicToJpeg, l as resizeToJpeg, o as hasAlphaChannel } from "./image-ops-CI-IGr62.js";
import { d as readLocalFileSafely, s as openFileWithinRoot, t as SafeOpenError } from "./fs-safe-CH-zulhg.js";
import { a as safeFileURLToPath, t as assertNoWindowsNetworkPath } from "./local-file-access-BpdUv1jJ.js";
import { c as resolveMediaBufferPath } from "./store-BuB7MfIQ.js";
...
async function resolveMediaStoreUriToPath(mediaUrl) {
	...
	try {
		...
		throw new LocalMediaAccessError("invalid-path", `Invalid media URI: ${mediaUrl}`, { cause: err });
	}
	if (parsed.hostname !== "inbound") throw new LocalMediaAccessError("path-not-allowed", `Unsupported media URI location: ${parsed.hostname || "(missing)"}`);
	let id;
	try {
	...
	if (!id || id.includes("/")) throw new LocalMediaAccessError("invalid-path", `Invalid media URI: ${mediaUrl}`);
	try {
		return await resolveMediaBufferPath(id, "inbound");
	} catch (err) {
		throw new LocalMediaAccessError("invalid-path", err instanceof Error ? err.message : `Invalid media URI: ${mediaUrl}`, { cause: err });
	}
}
