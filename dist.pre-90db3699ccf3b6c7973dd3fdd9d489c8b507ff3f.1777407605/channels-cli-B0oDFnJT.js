...
import { i as normalizeChannelId, r as listChannelPlugins } from "./registry-CaUS2Z2d.js";
...
import { n as listBundledPackageChannelMetadata } from "./bundled-package-channel-metadata-BM15fFzr.js";
import { n as resolveInstallableChannelPlugin } from "./channel-plugin-resolution-DbrzARDX.js";
import { t as formatCliChannelOptions } from "./channel-options-ELuRD1fP.js";
...
function loadChannelsCommands() {
	channelsCommandsPromise ??= import("./channels-BJbeoY95.js");
...
function registerChannelsCli(program) {
	...
	addChannelSetupOptions(channels.command("add").description("Add or update a channel account").option("--channel <name>", `Channel (${channelNames})`).option("--account <id>", "Account id (default when omitted)").option("--name <name>", "Display name for this account").option("--token <token>", "Channel token or credential payload").option("--token-file <path>", "Read channel token or credential payload from file").option("--bot-token <token>", "Bot token").option("--app-token <token>", "App token").option("--password <password>", "Channel password or login secret").option("--cli-path <path>", "Channel CLI path").option("--url <url>", "Channel setup URL").option("--http-url <url>", "Channel HTTP service URL").option("--use-env", "Use env-backed credentials when supported", false)).action(async (opts, command) => {
		await runChannelsCommand(async () => {
