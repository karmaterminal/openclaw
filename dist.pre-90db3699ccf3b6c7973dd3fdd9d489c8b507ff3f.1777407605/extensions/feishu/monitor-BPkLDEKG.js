...
import { a as resolveFeishuAccount, c as extractReplyText, d as normalizeString, f as parseCommentContentElements, h as normalizeCommentFileType, m as buildFeishuCommentTarget, n as listEnabledFeishuAccounts, o as resolveFeishuRuntimeAccount, p as readString$1, s as encodeQuery, u as isRecord$2 } from "./accounts-BtXgszRg.js";
import { a as resolveFeishuReplyPolicy, c as createFeishuCardInteractionEnvelope, l as decodeFeishuCardAction, n as resolveFeishuAllowlistMatch, r as resolveFeishuGroupConfig, s as buildFeishuCardActionTextFallback, t as isFeishuGroupAllowed } from "./policy-CrABD4B7.js";
import { t as getFeishuRuntime } from "./runtime-CG0DuRCy.js";
import { a as getFeishuUserAgent, i as createFeishuWSClient, n as createEventDispatcher, r as createFeishuClient } from "./client-cp9ZRF_-.js";
import { i as createCommentTypingReactionLifecycle, t as deliverCommentThreadText } from "./drive-sd4qCQuY.js";
import { buildAgentMediaPayload, createPersistentDedupe, createReplyPrefixContext, evaluateSupplementalContextVisibility, filterSupplementalContextItems, loadSessionStore, normalizeAgentId, resolveChannelContextVisibilityMode, resolveSessionStoreEntry } from "./runtime-api.js";
import { a as sendCardFeishu, c as sendStructuredCardFeishu, d as extractMentionTargets, f as isMentionForwardRequest, h as normalizeFeishuExternalKey, i as resolveFeishuCardTemplate, l as parsePostContent, m as sendMediaFeishu, n as getMessageFeishu, p as downloadMessageResourceFeishu, r as listFeishuThreadMessages, s as sendMessageFeishu, u as buildMentionedCardContent } from "./send-vMUvqjIt.js";
import { i as waitForAbortableDelay, r as raceWithTimeoutAndAbort, t as probeFeishu } from "./probe-ObkAMDUu.js";
...
import { WEBHOOK_ANOMALY_COUNTER_DEFAULTS, WEBHOOK_RATE_LIMIT_DEFAULTS, applyBasicWebhookRequestGuards, createFixedWindowRateLimiter, createWebhookAnomalyTracker } from "openclaw/plugin-sdk/webhook-ingress";
...
function parseFeishuCardActionEventPayload(value) {
	...
	if (!isRecord$2(operator) || !isRecord$2(action) || !isRecord$2(context)) return null;
	const token = readString$1(value.token);
	const openId = readString$1(operator.open_id);
	const userId = readString$1(operator.user_id);
	const unionId = readString$1(operator.union_id);
	const tag = readString$1(action.tag);
	const actionValue = action.value;
	const contextOpenId = readString$1(context.open_id);
	const contextUserId = readString$1(context.user_id);
	const chatId = readString$1(context.chat_id);
	if (!token || !openId || !userId || !unionId || !tag || !isRecord$2(actionValue) || !contextOpenId || !contextUserId || !chatId) return null;
	return {
		operator: {
			open_id: openId,
			user_id: userId,
