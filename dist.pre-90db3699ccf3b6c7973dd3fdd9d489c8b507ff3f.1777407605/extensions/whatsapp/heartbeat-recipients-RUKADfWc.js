import { m as resolveWhatsAppAccount, p as resolveDefaultWhatsAppAccountId, s as toWhatsappJid } from "./text-runtime-pSZYjiCV.js";
import { t as WHATSAPP_LEGACY_OUTBOUND_SEND_DEP_KEYS } from "./outbound-send-deps-DYLgiJmX.js";
import { i as lookupInboundMessageMetaForTarget } from "./quoted-message-BnVl5P4C.js";
...
function createWhatsAppOutboundBase({ chunker, sendMessageWhatsApp, sendPollWhatsApp, shouldLogVerbose, resolveTarget, normalizeText = (text) => text ?? "", skipEmptyText = false }) {
	...
	return {
		...
		...createAttachedChannelResultAdapter({
			...
			sendMedia: async ({ cfg, to, text, mediaUrl, mediaAccess, mediaLocalRoots, mediaReadFile, accountId, deps, gifPlayback, replyToId }) => {
				const send = resolveOutboundSendDep(deps, "whatsapp", { legacyKeys: WHATSAPP_LEGACY_OUTBOUND_SEND_DEP_KEYS }) ?? sendMessageWhatsApp;
				const quotedMessageKey = resolveQuotedMessageKey({
					accountId: resolveQuoteLookupAccountId(cfg, accountId),
					to,
...
function resolveWhatsAppHeartbeatRecipients(cfg, opts = {}) {
	...
	const resolvedAccountId = opts.accountId?.trim() || resolveDefaultWhatsAppAccountId(cfg) || DEFAULT_ACCOUNT_ID$2;
