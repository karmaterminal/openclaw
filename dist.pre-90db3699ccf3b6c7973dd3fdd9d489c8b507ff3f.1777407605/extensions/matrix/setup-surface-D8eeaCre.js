...
import { r as moveSingleMatrixAccountConfigToNamedAccount, t as resolveMatrixSetupDmAllowFrom } from "./setup-dm-policy-Y6kVwV54.js";
import { t as resolveMatrixConfigFieldPath } from "./config-paths-CzXbnXGG.js";
import { n as updateMatrixAccountConfig } from "./config-update-CGejYzc0.js";
import { n as validateMatrixHomeserverUrl, r as isPrivateOrLoopbackHost, t as resolveValidatedMatrixHomeserverUrl } from "./url-validation-Bl0WGrAg.js";
import { i as isMatrixSdkAvailable, r as ensureMatrixSdkInstalled } from "./deps-9J1Viz26.js";
...
async function promptMatrixAllowFrom(params) {
	...
	while (true) {
		...
		if (pending.length > 0) {
			const { resolveMatrixTargets } = await import("./resolve-targets-DSeGOM8Y.js").then((n) => n.n);
	...
	if (accessConfig) if (accessConfig.policy !== "allowlist") next = setMatrixGroupPolicy(next, accessConfig.policy, params.accountId);
		...
		if (accessConfig.entries.length > 0) try {
			...
			for (const entry of accessConfig.entries) {
				...
				const { listMatrixDirectoryGroupsLive } = await import("./directory-live-CA15qJVq.js").then((n) => n.t);
...
const matrixOnboardingAdapter = {
	...
	afterConfigWritten: async ({ previousCfg, cfg, accountId, runtime }) => {
		const { runMatrixSetupBootstrapAfterConfigWrite } = await import("./setup-bootstrap-JqvYDoWO.js");
