...
import { d as buildModelsKeyboard, g as resolveModelSelection, h as parseModelCallbackData, m as getModelsPageSize, o as buildTelegramModelsMenuButtons, p as calculateTotalPages, t as buildCommandsPaginationKeyboard } from "./command-ui-Du_lvsrz.js";
import { i as resolveTelegramCustomCommands, r as normalizeTelegramCommandName, t as TELEGRAM_COMMAND_NAME_PATTERN } from "./command-config-Dt5ZeWHg.js";
import { t as createTelegramThreadBindingManager } from "./thread-bindings-D6IlH-ew.js";
import { n as cacheSticker, t as describeStickerImage } from "./sticker-cache-BrsCFplN.js";
import { n as evaluateTelegramGroupPolicyAccess, t as evaluateTelegramGroupBaseAccess } from "./group-access-BsAQw3WM.js";
import { n as deliverReplies, r as emitInternalMessageSentHook, t as resolveMedia } from "./delivery-B8DudFfH.js";
import { applyModelOverrideToSessionEntry, isNativeCommandsExplicitlyDisabled, loadConfig, loadSessionStore, loadSessionStore as loadSessionStore$1, resolveChannelContextVisibilityMode, resolveChannelGroupPolicy, resolveChannelGroupRequireMention, resolveMarkdownTableMode as resolveMarkdownTableMode$1, resolveNativeCommandsEnabled, resolveNativeSkillsEnabled, resolveSessionStoreEntry, resolveSessionStoreEntry as resolveSessionStoreEntry$1, resolveStorePath, updateSessionStore, writeConfigFile } from "openclaw/plugin-sdk/config-runtime";
...
import { findCodeRegions, isInsideCode, normalizeLowercaseStringOrEmpty, normalizeOptionalLowercaseString, normalizeOptionalString, readStringValue, stripReasoningTagsFromText } from "openclaw/plugin-sdk/text-runtime";
...
import { buildMentionRegexes, createInboundDebouncer, formatInboundEnvelope, formatLocationText, implicitMentionKindWhen, logInboundDrop, matchesMentionWithExplicit, resolveEnvelopeFormatOptions, resolveInboundDebounceMs, resolveInboundMentionDecision, shouldDebounceTextInbound, toLocationContext } from "openclaw/plugin-sdk/channel-inbound";
...
import { createDedupeCache, enqueueSystemEvent, parseExecApprovalCommandText } from "openclaw/plugin-sdk/infra-runtime";
import { MediaFetchError, getAgentScopedMediaLocalRoots } from "openclaw/plugin-sdk/media-runtime";
import { loadWebMedia } from "openclaw/plugin-sdk/web-media";
import fs from "node:fs";
import path from "node:path";
import { generateConversationLabel, isAbortRequestText, isBtwRequestText, resolveChunkMode } from "openclaw/plugin-sdk/reply-runtime";
...
async function loadTelegramSendRuntime() {
	telegramSendRuntimePromise ??= import("./send-trgrTBon.js").then((n) => n.p);
...
async function loadTelegramNativeCommandDeliveryRuntime() {
	telegramNativeCommandDeliveryRuntimePromise ??= import("./bot-native-commands.delivery.runtime-ByelCeWB.js");
...
async function loadTelegramMessageContextSessionRuntime(runtime) {
	...
	return {
		...await import("./bot-message-context.session.runtime-Bj8MZr6l.js"),
		...runtime
	};
}
...
async function loadTelegramMessageContextRuntime() {
	telegramMessageContextRuntimePromise ??= import("./bot-message-context.runtime-z6yA5pOv.js");
...
function createTelegramBot(opts) {
	...
	const maybePersistSafeWatermark = () => {
		...
		if (failedUpdateIds.size > 0) {
			let minFailed = null;
			for (const id of failedUpdateIds) if (minFailed === null || id < minFailed) minFailed = id;
			if (minFailed !== null) safe = Math.min(safe, minFailed - 1);
		}
		if (highestPersistedUpdateId !== null && safe <= highestPersistedUpdateId) return;
		highestPersistedUpdateId = safe;
		Promise.resolve().then(() => opts.updateOffset?.onUpdateId?.(safe)).catch((err) => {
	...
	bot.use(async (ctx, next) => {
		...
		if (updateKey) {
			if (pendingUpdateKeys.has(updateKey) || recentUpdates.peek(updateKey)) {
				...
				if (typeof updateId === "number") pendingUpdateIds.delete(updateId);
				return;
			}
