...
const CODEX_AUTH_ENV_CLEAR_KEYS = [];
...
async function prepareCopiedCodexHome(params) {
	...
	if (configToml) await writePrivateSecretFileAtomic({
		rootDir: params.agentDir,
...
async function writeCodexAcpWrapper(params) {
	await fs.mkdir(path.dirname(params.wrapperPath), {
		recursive: true,
		mode: 448
	});
...
`;
	await fs.writeFile(params.wrapperPath, content, { mode: 448 });
	await fs.chmod(params.wrapperPath, 448);
	return shellArg(params.wrapperPath);
}
async function prepareAcpxCodexAuthConfig(params) {
	if (params.pluginConfig.agents[CODEX_AGENT_ID]) return params.pluginConfig;
