...
import { B as resolveMemoryLightDreamingConfig, F as resolveMemoryDeepDreamingConfig, M as formatMemoryDreamingDay, P as resolveMemoryCorePluginConfig, V as resolveMemoryRemDreamingConfig, z as resolveMemoryDreamingWorkspaces } from "./dreaming-C4iELShs.js";
import { o as parseUsageCountedSessionIdFromFileName } from "./artifacts-Czkc6wYi.js";
import { n as resolveHeartbeatIntervalMs, t as isHeartbeatEnabledForAgent } from "./heartbeat-summary-Bi99g3ud.js";
import { s as peekSystemEventEntries } from "./system-events-B-8iLjB8.js";
import "./text-runtime-ITCc6m8o.js";
import "./config-runtime-ByTE4rnO.js";
import "./infra-runtime-DIh3Zemf.js";
import { n as appendMemoryHostEvent } from "./events-BhSyzOQ5.js";
import "./memory-core-host-status-B3E7Ssz5.js";
import { c as buildSessionEntry, d as normalizeSessionTranscriptPathForComparison, f as sessionPathForFile, l as listSessionFilesForAgent, u as loadDreamingNarrativeTranscriptPathSetForAgent } from "./engine-qmd-DaN67D_R.js";
import "./memory-core-host-engine-qmd-DQwwvNyf.js";
import "./memory-host-events-_6QK-b06.js";
import { n as withTrailingNewline, t as replaceManagedMarkdownBlock } from "./memory-host-markdown-8dIKZECF.js";
import { a as readShortTermRecallEntries, c as recordShortTermRecalls, i as rankShortTermPromotionCandidates, n as applyShortTermPromotions, o as recordDreamingPhaseSignals, u as repairShortTermPromotionArtifacts } from "./short-term-promotion-CbMijT_r.js";
import { n as normalizeTrimmedString, t as includesSystemEventToken } from "./dreaming-shared-CsighQz4.js";
import { r as generateAndAppendDreamNarrative, t as buildNarrativeSessionKey } from "./dreaming-narrative-CqAsjqgv.js";
...
async function collectSessionIngestionBatches(params) {
	...
	for (const agentId of agentIds) {
		...
		const dreamingTranscriptPaths = files.length > 0 ? loadDreamingNarrativeTranscriptPathSetForAgent(agentId) : /* @__PURE__ */ new Set();
		for (const absolutePath of files) sessionFiles.push({
			agentId,
			absolutePath,
			generatedByDreamingNarrative: dreamingTranscriptPaths.has(normalizeSessionTranscriptPathForComparison(absolutePath)),
	...
	for (const file of sortedFiles) {
		...
		const entry = await buildSessionEntry(file.absolutePath, { generatedByDreamingNarrative: file.generatedByDreamingNarrative });
		if (!entry) continue;
...
const CRON_SESSION_TARGET_MAIN = "main";
...
function buildManagedDreamingPatch(job, desired) {
	...
	if (normalizeLowercaseStringOrEmpty(normalizeTrimmedString(job.sessionTarget)) !== "main") patch.sessionTarget = "main";
...
function resolveDreamingBlockedReason(cfg) {
	if (!resolveShortTermPromotionDreamingConfig({
		pluginConfig: resolveMemoryCorePluginConfig(cfg),
		cfg
	}).enabled) return null;
	const defaultAgentId = resolveDefaultAgentId(cfg);
	const enabledForDefault = isHeartbeatEnabledForAgent(cfg, defaultAgentId);
	const intervalMs = resolveHeartbeatIntervalMs(cfg, void 0, cfg.agents?.defaults?.heartbeat);
	if (enabledForDefault && intervalMs != null) return null;
...
function registerShortTermPromotionDreaming(api) {
	...
	api.on("before_agent_reply", async (event, ctx) => {
		try {
			...
			if (!hasPendingManagedDreamingCronEvent(ctx.sessionKey) || !includesSystemEventToken(event.cleanedBody, DREAMING_SYSTEM_EVENT_TEXT)) return;
...
//#endregion
export { seedHistoricalDailyMemorySignals as a, previewRemDreaming as i, resolveDreamingBlockedReason as n, resolveShortTermPromotionDreamingConfig as r, registerShortTermPromotionDreaming as t };
