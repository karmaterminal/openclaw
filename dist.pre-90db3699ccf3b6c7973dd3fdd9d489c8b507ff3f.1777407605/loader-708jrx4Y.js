import { a as normalizeLowercaseStringOrEmpty, c as normalizeOptionalString, d as normalizeStringifiedOptionalString } from "./string-coerce-CjxCKZ6B.js";
...
import { i as openBoundaryFileSync } from "./boundary-file-read-BSd9MXPy.js";
import { l as normalizeTrimmedStringList } from "./string-normalization-5F-0OqbT.js";
import { i as safeStatSync, n as isPathInside } from "./path-safety-Ba_J5G0M.js";
import { n as discoverOpenClawPlugins, r as resolvePluginCacheInputs } from "./discovery-DoO9rWnu.js";
import { h as normalizeChannelMeta } from "./bundled-Djgswlqd.js";
import { f as resolvePluginRuntimeModulePath, g as shouldPreferNativeJiti } from "./sdk-alias-Dij9SbK3.js";
...
import { i as kindsEqual, n as defaultSlotIdForKey, r as hasKind } from "./slots-DkFhJWYQ.js";
import { c as resolveEffectivePluginActivationState, n as createPluginActivationSource, o as normalizePluginsConfig, s as resolveEffectiveEnableState, t as applyTestPluginDefaults, u as resolveMemorySlotDecision } from "./config-state-COejNgwx.js";
...
import { t as unwrapDefaultModuleExport } from "./module-export-DDzOnsga.js";
import { c as listChatChannels } from "./registry-CGsqKAN1.js";
import { t as validateJsonSchemaValue } from "./schema-validator-BkLQHQcA.js";
import { a as getRuntimeConfigSourceSnapshot, r as getRuntimeConfigSnapshot } from "./runtime-snapshot-C54O1-gI.js";
import { t as normalizePluginGatewayMethodScope } from "./gateway-method-policy-C7rf0SeV.js";
import { a as hasExplicitPluginIdScope, c as serializePluginIdScope, i as createPluginIdScopeSet, s as normalizePluginIdScope } from "./persisted-auth-state-Cl6A4BLr.js";
import { t as buildPluginApi } from "./api-builder-d3jBS7ML.js";
import { t as isChannelConfigured } from "./channel-configured-CdNwff8K.js";
import { I as resolveMemoryDreamingConfig, L as resolveMemoryDreamingPluginConfig, _ as DEFAULT_MEMORY_DREAMING_PLUGIN_ID } from "./dreaming-C4iELShs.js";
import { a as restoreDetachedTaskLifecycleRuntimeRegistration, i as registerDetachedTaskLifecycleRuntime, n as getDetachedTaskLifecycleRuntimeRegistration, t as clearDetachedTaskLifecycleRuntimeRegistration } from "./detached-task-runtime-state-BIBcUMTx.js";
import { n as inspectBundleMcpRuntimeSupport } from "./bundle-mcp-E1RHhsMJ.js";
import { a as clearPluginCommands, i as validatePluginCommandDefinition, n as registerPluginCommand, o as clearPluginCommandsForPlugin } from "./command-registration-CF50YQAH.js";
import { i as initializeGlobalHookRunner } from "./hook-runner-global-DuCZiaZE.js";
import { a as isPromptInjectionHookName, c as clearPluginInteractiveHandlers, i as isPluginHookName, l as clearPluginInteractiveHandlersForPlugin, s as stripPromptMutationFieldsFromLegacyHookResult, u as registerPluginInteractiveHandler } from "./types-CrIHEfdc.js";
import { a as registerMemoryEmbeddingProvider, i as listRegisteredMemoryEmbeddingProviders, n as getRegisteredMemoryEmbeddingProvider, o as restoreRegisteredMemoryEmbeddingProviders } from "./memory-embedding-providers-BZk7dl3-.js";
import { a as getMemoryPromptSectionBuilder, d as registerMemoryCapability, f as registerMemoryCorpusSupplement, g as registerMemoryRuntime, h as registerMemoryPromptSupplement, i as getMemoryFlushPlanResolver, l as listMemoryCorpusSupplements, m as registerMemoryPromptSection, n as clearMemoryPluginState, o as getMemoryRuntime, p as registerMemoryFlushPlanResolver, r as getMemoryCapabilityRegistration, u as listMemoryPromptSupplements, v as restoreMemoryPluginState } from "./memory-state-By3bF34d.js";
import { i as withProfile } from "./plugin-load-profile-kP6V6eFL.js";
import { r as registerContextEngineForOwner, t as clearContextEnginesForOwner } from "./registry-CJ9kiBp3.js";
import { f as registerInternalHook, h as unregisterInternalHook } from "./internal-hooks-CpgiJgbB.js";
import { i as NODE_SYSTEM_RUN_COMMANDS, n as NODE_EXEC_APPROVALS_COMMANDS, r as NODE_SYSTEM_NOTIFY_COMMAND } from "./node-commands-CUkVM3Gb.js";
import { t as createEmptyPluginRegistry } from "./registry-empty-BzdJBxDW.js";
import { a as getActivePluginRegistry, l as getActivePluginRuntimeSubagentMode, o as getActivePluginRegistryKey, p as recordImportedPluginId, x as setActivePluginRegistry } from "./runtime-Cym-fqI8.js";
import { a as normalizePluginHttpPath, t as findOverlappingPluginHttpRoute } from "./http-route-overlap-CBIhSNNb.js";
import { r as withPluginRuntimePluginIdScope } from "./gateway-request-scope-Cc9jmc-3.js";
...
//#endregion
//#region src/plugins/embedded-extension-factory.ts
function listEmbeddedExtensionFactories() {
...
function createPluginRegistry(registryParams) {
	...
	const registerPiEmbeddedExtensionFactory = (record, factory) => {
		...
		if (!(record.contracts?.embeddedExtensionFactories ?? []).includes("pi")) {
			pushDiagnostic({
				level: "error",
				pluginId: record.id,
				source: record.source,
				message: "plugin must declare contracts.embeddedExtensionFactories: [\"pi\"] to register Pi embedded extension factories"
			});
			return;
		}
		...
		if (registry.embeddedExtensionFactories.some((entry) => entry.pluginId === record.id && entry.rawFactory === factory)) return;
		const safeFactory = async (pi) => {
			try {
				await factory(pi);
			} catch (error) {
				const detail = error instanceof Error ? error.message : String(error);
				registryParams.logger.warn(`[plugins] embedded extension factory failed for ${record.id}: ${detail}`);
			}
		};
...
function createPluginJitiLoader(options) {
	...
	return (modulePath) => {
		const tryNative = shouldPreferNativeJiti(modulePath) && !isBundledRuntimeDependencyMirrorPath(modulePath);
...
function isBundledRuntimeDependencyMirrorPath(modulePath) {
	const resolvedModulePath = path.resolve(modulePath);
	for (const nodeModulesDir of registeredBundledRuntimeDepNodePaths) {
		const installRoot = path.dirname(nodeModulesDir);
		if (resolvedModulePath === installRoot || resolvedModulePath.startsWith(`${installRoot}${path.sep}`)) return true;
	}
	return false;
}
function registerBundledRuntimeDependencyNodePath(installRoot) {
...
function loadOpenClawPlugins(options = {}) {
	...
	try {
		...
		for (const candidate of orderedCandidates) {
			...
			const scopedSetupOnlyChannelPluginRequested = includeSetupOnlyChannelPlugins && !validateOnly && onlyPluginIdSet && manifestRecord.channels.length > 0 && (!enableState.enabled || forceSetupOnlyChannelPlugins);
			const registrationMode = scopedSetupOnlyChannelPluginRequested && (!requireSetupEntryForSetupOnlyChannelPlugins || Boolean(manifestRecord.setupSource)) ? "setup-only" : scopedSetupOnlyChannelPluginRequested && requireSetupEntryForSetupOnlyChannelPlugins ? null : enableState.enabled ? shouldLoadModules && !validateOnly && shouldLoadChannelPluginInSetupRuntime({
			...
			if (shouldLoadModules && candidate.origin === "bundled" && enableState.enabled) try {
				...
				if (depsInstallResult.installedSpecs.length > 0) {
					...
					if (shouldActivate) logger.info(`[plugins] ${record.id} installed bundled runtime deps: ${depsInstallResult.installedSpecs.join(", ")}`);
				}
			...
			const opened = openBoundaryFileSync({
				absolutePath: resolveCanonicalDistRuntimeSource((registrationMode === "setup-only" || registrationMode === "setup-runtime") && runtimeSetupSource ? runtimeSetupSource : runtimeCandidateSource),
			...
			if ((registrationMode === "setup-only" || registrationMode === "setup-runtime") && manifestRecord.setupSource) {
				const setupRegistration = resolveSetupChannelRegistration(mod, { installRuntimeDeps: enableState.enabled });
				...
				if (setupRegistration.plugin) {
					...
					if (registrationMode === "setup-runtime" && setupRegistration.usesBundledSetupContract && runtimeCandidateSource !== safeSource) {
...
//#endregion
export { resolveRuntimePluginRegistry as a, getCompactionProvider as c, listRegisteredAgentHarnesses as d, resetRegisteredAgentHarnessSessions as f, resolveCompatibleRuntimePluginRegistry as i, resolvePluginActivationSourceConfig as l, loadOpenClawPluginCliRegistry as n, listEmbeddedExtensionFactories as o, loadOpenClawPlugins as r, listCodexAppServerExtensionFactories as s, isPluginRegistryLoadInFlight as t, disposeRegisteredAgentHarnesses as u };
