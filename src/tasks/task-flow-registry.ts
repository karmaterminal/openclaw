...
function createFlowSnapshotWith(
  next?: TaskFlowRecord | readonly TaskFlowRecord[],
  ...
  if (next) {
    for (const flow of Array.isArray(next) ? next : [next]) {
      snapshot.set(flow.flowId, cloneFlowRecord(flow));
    }
  }
  return snapshot;
}
