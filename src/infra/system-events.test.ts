...
describe("system events (session routing)", () => {
  ...
  it("neutralizes nested system markers before formatting queued events", async () => {
    // Anti-spoof property (#858) now lives unconditionally at the queue
    // boundary: every enqueued event is run through `sanitizeInboundSystemTags`,
    // so nested `[System]` bracket-tags and `System:` line-prefixes are
    // neutralized in the STORED entry before any drain/heartbeat render path can
    // surface them. This is strictly stronger than the old trust-gated guard
    // (which only sanitized owner-downgraded producers).
    ...
    // Stored entry is already sanitized at enqueue time.
    expect(peekSystemEvents(key)).toEqual([
    ...
    // Prefix spoof form `System:` rewritten to `System (untrusted):`, then
    // wrapped by the drain-layer `System:` prefix.
    expect(result).toContain("System: System (untrusted): second instruction");
    // Original spoof-vector substrings MUST NOT survive into the rendered output.
