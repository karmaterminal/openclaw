...
export function resolveContinuationRuntimeConfig(
  ...
  return {
    ...
    defaultDelayMs: clampNonNegativeInt(
      continuation?.defaultDelayMs,
      DEFAULT_CONTINUATION_DELAY_MS,
    ),
    minDelayMs: clampNonNegativeInt(continuation?.minDelayMs, DEFAULT_CONTINUATION_MIN_DELAY_MS),
    maxDelayMs: clampNonNegativeInt(continuation?.maxDelayMs, DEFAULT_CONTINUATION_MAX_DELAY_MS),
