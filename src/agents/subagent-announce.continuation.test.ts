...
import {
  seedSessionStore,
  updateSeededSessionStore,
} from "../config/sessions/store-seed.test-helpers.js";
...
/**
 * Seed the session store through the canonical SQLite-backed store API.
 * (Shared helper: tracks the production store seam, drift-proof vs raw-JSON.)
 */
async function writeSessionStore(data: Record<string, unknown>) {
  await seedSessionStore(data);
}
...
describe("subagent announce continuation chaining", () => {
  ...
  async function runContinuationAnnounce(params: {
    ...
    // Inject the child entry into the seeded store via the canonical API.
    await updateSeededSessionStore((store) => {
