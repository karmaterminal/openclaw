...
vi.mock("./subagent-registry.js", () => ({
  ...
  resolveRequesterForChildSession: (key: string) =>
    mocked.resolveRequesterForChildSessionMock(key),
...
describe("subagent announce continuation chaining", () => {
  ...
  beforeEach(() => {
    ...
    setRuntimeConfigSnapshot(makeBaseConfig() as any);
  });
  ...
  it("reads generationGuardTolerance when the delayed chain-hop timer fires", async () => {
    ...
    setRuntimeConfigSnapshot(
      makeBaseConfig({ maxChainLength: 3, generationGuardTolerance: 3 }) as any,
    );
