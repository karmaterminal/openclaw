...
describe("continue_work tool — delaySeconds boundary validation", () => {
  ...
  it("rejects negative delaySeconds with ToolInputError", async () => {
    ...
    const result = await tool.execute("call-negative", {
    ...
    // The error surface should explicitly mention "non-negative" so the
    // model can self-correct on the next turn rather than guessing.
    expect(result).toMatchObject({
      isError: true,
      content: expect.arrayContaining([
        expect.objectContaining({
          text: expect.stringContaining("non-negative"),
        }),
      ]),
    });
  });
