...
export async function loadSubagentAnnounceModule(): Promise<SubagentAnnounceModule> {
...
// Upstream 2167eab4cf added these two as the direct, non-injected entry points and
// its own new modules (subagent-registry-restore.ts, subagent-registry-context-cleanup.ts)
// import them by name. Exported here rather than reimplemented so there is exactly one
// binding and one loader, shared with the injected default below.
