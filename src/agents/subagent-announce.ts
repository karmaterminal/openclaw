...
async function drainChildContinuationQueue(params: {
  ...
  try {
    ...
    const childEntry = loadSessionEntryByKey(params.childSessionKey) as
      ...
      | undefined;
    const dispatchConfig = subagentAnnounceDeps.resolveContinuationRuntimeConfig(params.cfg);
...
function loadEnabledContinuationDrainConfig(params: {
  childSessionKey: string;
}): ReturnType<typeof subagentAnnounceDeps.getRuntimeConfig> | undefined {
  ...
  return cfg?.agents?.defaults?.continuation?.enabled === true ? cfg : undefined;
}
...
export async function runSubagentAnnounceFlow(params: {
  ...
  try {
    ...
    const continuationDrainConfig = loadEnabledContinuationDrainConfig({
      childSessionKey: params.childSessionKey,
    });
    if (continuationDrainConfig) {
      await drainChildContinuationQueue({
        childSessionKey: params.childSessionKey,
        requesterOrigin: targetRequesterOrigin,
        cfg: continuationDrainConfig,
      });
    }
