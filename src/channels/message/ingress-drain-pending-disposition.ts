...
type ResolvePendingDispositionsParams<TPayload, TMetadata, TCompletedMetadata> = Omit<
  ApplyPendingDispositionsParams<TPayload, TMetadata, TCompletedMetadata>,
  "pending"
> & {
...
export async function resolveIngressPendingDispositions<TPayload, TMetadata, TCompletedMetadata>(
  params: ResolvePendingDispositionsParams<TPayload, TMetadata, TCompletedMetadata>,
): Promise<AppliedIngressPendingDispositions<TPayload, TMetadata>> {
  const pending = await params.queue.listPending({ limit: "all", orderBy: params.orderBy });
  if (!params.resolvePendingDisposition) {
    return { pending, blockedLaneKeys: new Set() };
  }
  return await applyIngressPendingDispositions({ ...params, pending });
}
...
export async function applyIngressPendingDispositions<TPayload, TMetadata, TCompletedMetadata>(
  ...
  for (const event of params.pending) {
    ...
    const failed = await failPendingRecord(params.queue, event, disposition, params.dispositionNow);
    if (!failed) {
