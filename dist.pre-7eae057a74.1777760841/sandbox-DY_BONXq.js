	...
	if (workspaceDir === sandboxWorkspaceDir) {
		...
		if (cfg.workspaceAccess !== "rw") try {
			const [{ getRemoteSkillEligibility }, { canExecRequestNode }, { syncSkillsToWorkspace }] = await Promise.all([
				import("./skills-remote-Cni_uoL-.js"),
				import("./exec-defaults-DKdE02aU.js"),
				import("./skills-v3xfeU6p.js")
			]);
...
async function resolveSandboxContext(params) {
	...
	if (cfg.prune.idleHours !== 0 || cfg.prune.maxAgeDays !== 0) await (await import("./prune-BSRmBV9Z.js")).maybePruneSandboxes(cfg);
	...
	const bridgeAuth = cfg.browser.enabled ? await (async () => {
		const cfgForAuth = params.config ?? (await import("./config-Bj8Cp5-n.js")).loadConfig();
