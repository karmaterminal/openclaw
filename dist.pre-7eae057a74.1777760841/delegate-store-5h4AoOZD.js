...
import { i as failFlow, o as finishFlow, r as deleteTaskFlowRecordById, t as createManagedTaskFlow, u as listTaskFlowsForOwnerKey } from "./task-flow-runtime-internal-CoeDxxCU.js";
import { z } from "zod";
...
const PendingDelegateModeSchema = z.enum([
	"normal",
	"silent",
	"silent-wake",
	"post-compaction"
]);
const PendingDelegateStateBaseSchema = z.object({
...
const LegacyPendingDelegateStateSchema = z.object({
	kind: z.literal("continuation_delegate"),
	task: z.string().min(1),
	delayMs: z.number().int().nonnegative().optional(),
	...
	if ([
		state.silent,
		state.silentWake,
		state.postCompaction
	].filter(Boolean).length > 1 && !(isLegacySilentWake && state.postCompaction !== true)) ctx.addIssue({
...
const PendingDelegateStateWireSchema = z.union([PendingDelegateStateSchema, LegacyPendingDelegateStateSchema]);
z.object({
	kind: z.literal("continuation_delegate"),
	task: z.string().min(1),
	delayMs: z.number().int().nonnegative().optional(),
...
function flowToDelegate(flow, state) {
	const mode = state.mode === "normal" ? void 0 : state.mode;
	return {
		task: state.task,
		...state.delayMs !== void 0 ? { delayMs: state.delayMs } : {},
		...mode !== void 0 ? { mode } : {},
