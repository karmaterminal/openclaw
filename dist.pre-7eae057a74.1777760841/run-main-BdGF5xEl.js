...
import { i as normalizeEnv, t as isTruthyEnvValue } from "./env-CcBRbhYW.js";
import { _ as resolveStateDir } from "./paths-CrDgDBYA.js";
import { t as assertSupportedRuntime } from "./runtime-guard-8ZmDMOLK.js";
...
import { i as initializeDebugProxyCapture, r as finalizeDebugProxyCapture } from "./runtime-A0C_9pqU.js";
import { r as ensureGlobalUndiciEnvProxyDispatcher } from "./undici-global-dispatcher-LU0hItIA.js";
import { t as ensureOpenClawCliOnPath } from "./path-env-DQoBHrF0.js";
import { i as shouldSkipPluginCommandRegistration, n as shouldRegisterPrimaryCommandOnly } from "./command-registration-policy-D1vo-wQJ.js";
import { a as shouldEnsureCliPathForCommandPath, c as cliCommandCatalog, n as ensureCliExecutionBootstrap, o as resolveCliCommandPathPolicy, r as resolveCliExecutionStartupContext, s as matchesCommandPath, t as applyCliExecutionStartupPresentation } from "./command-execution-startup-Bc0f3eCG.js";
import { n as maybeWarnAboutDebugProxyCoverage } from "./coverage-EJiX_X4c.js";
...
function loadConfigCli() {
	configCliPromise ??= import("./config-cli-227ad7Hd.js");
	return configCliPromise;
}
function loadModelsList() {
	modelsListPromise ??= import("./list-Cx4avaCU.js");
...
const routedCommandDefinitions = {
	health: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { healthCommand } = await import("./health-l67cxIle.js");
	...
	status: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			if (args.json) {
				const { statusJsonCommand } = await import("./status-json-E95YGRzE.js");
			...
			const { statusCommand } = await import("./status-CGHsijhn.js");
	...
	"gateway-status": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { runDaemonStatus } = await import("./status-B-OtjzA1.js");
	...
	sessions: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { sessionsCommand } = await import("./sessions-CZSc7D1z.js");
	...
	"agents-list": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { agentsListCommand } = await import("./agents-sRJFfbwG.js");
	...
	"channels-list": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { channelsListCommand } = await import("./list-BlIcB2Y6.js");
	...
	"channels-status": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { channelsStatusCommand } = await import("./status-DHOLJNMG.js");
...
async function closeCliMemoryManagers() {
	...
	try {
		const { closeActiveMemorySearchManagers } = await import("./memory-runtime-5hDBGros.js");
...
async function runCli(argv = process$1.argv) {
	...
	if (shouldLoadCliDotEnv()) {
		const { loadCliDotEnv } = await import("./dotenv-C5-iDnbz.js");
	...
	try {
		if (shouldUseRootHelpFastPath(normalizedArgv)) {
			const { outputPrecomputedRootHelpText } = await import("./root-help-metadata-tbA2AnMH.js");
			if (!outputPrecomputedRootHelpText()) {
				const { outputRootHelp } = await import("./root-help-GnL5stc5.js");
		...
		if (primary && shouldRegisterPrimaryCommandOnly(parseArgv)) {
			const { getProgramContext } = await import("./program-context-_qCUzZTU.js");
			const ctx = getProgramContext(program);
			if (ctx) {
				const { registerCoreCliByName } = await import("./command-registry-BwHp_ul5.js");
				await registerCoreCliByName(program, ctx, primary, parseArgv);
			}
			const { registerSubCliByName } = await import("./register.subclis-IgMZcvuR.js");
