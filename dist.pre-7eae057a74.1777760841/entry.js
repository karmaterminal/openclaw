...
function tryHandleRootVersionFastPath(argv, deps = {}) {
	...
	(deps.resolveVersion ?? (async () => {
		const [{ VERSION }, { resolveCommitHash }] = await Promise.all([import("./version-BAqscPtH.js"), import("./git-commit-BPc2Wiw3.js")]);
		return {
			VERSION,
			resolveCommitHash
		};
...
function tryHandleRootHelpFastPath(argv, deps = {}) {
	...
	import("./root-help-metadata-tbA2AnMH.js").then(async ({ outputPrecomputedRootHelpText }) => {
...
function runMainOrRootHelp(argv) {
	...
	import("./run-main-BdGF5xEl.js").then(({ runCli }) => runCli(argv)).catch((error) => {
