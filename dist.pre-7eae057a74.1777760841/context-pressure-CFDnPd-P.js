...
*/
function resolveContextPressureBand(ratio, threshold, earlyWarningBand = 0) {
	if (!Number.isFinite(ratio) || ratio < 0 || !Number.isFinite(threshold) || threshold <= 0) return 0;
	const thresholdPct = Math.round(threshold * 100);
	const pressureBands = [
		...earlyWarningBand > 0 ? [{
			threshold: threshold * earlyWarningBand,
			band: Math.round(threshold * earlyWarningBand * 100)
		}] : [],
		{
			threshold,
			band: thresholdPct
...
function checkSessionContextPressure(params) {
	const { sessionEntry, sessionKey, contextPressureThreshold, contextWindowTokens, postCompaction = false, earlyWarningBand = 0 } = params;
	...
	if (!postCompaction && band === 0 && ratio < threshold) {
		if (log.isEnabled("debug")) log.debug(`[context-pressure:noop] reason=below-all-bands ratio=${Math.round(ratio * 100)}% threshold=${Math.round(threshold * 100)}% earlyWarningBand=${earlyWarningBand} rawRatio=${ratio.toFixed(4)} rawThreshold=${threshold.toFixed(4)} session=${sessionKey}`);
		return {
			fired: false,
			band: 0
		};
...
function checkTokenContextPressure(params) {
	const { sessionKey, totalTokens, contextWindow, threshold, postCompaction = false, earlyWarningBand = 0 } = params;
	...
	if (band === 0 && ratio < threshold) {
		if (log.isEnabled("debug")) log.debug(`[context-pressure:noop] reason=below-all-bands ratio=${percentUsed}% threshold=${Math.round(threshold * 100)}% earlyWarningBand=${earlyWarningBand} rawRatio=${ratio.toFixed(4)} rawThreshold=${threshold.toFixed(4)} session=${sessionKey}`);
		return null;
	}
