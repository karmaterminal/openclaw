...
**Side-effect warning for anyone repeating this.** `npx gitnexus analyze` **writes to the checkout**: it appended a 45-line `<!-- gitnexus:start -->` block to the repository's **tracked** root `AGENTS.md`, and created an untracked `.agents/skills/gitnexus/` tree. Neither was requested. `AGENTS.md` was reverted (`git checkout -- AGENTS.md`); the untracked directory was left in place without touching `.git/info/exclude`, since worktrees share the main `.git` and excluding there would affect sibling lanes. Both commits on this branch were verified to contain only this report. In a shared checkout, expect that `AGENTS.md` write and revert it before committing.
