...
2. move the three request-compaction completion/release functions out of
   `src/auto-reply/reply/agent-runner-execution.ts` and into the existing
   feature-owned `src/auto-reply/continuation/post-compaction-release.ts`.
...
failure isolation. The existing feature-owned
`src/auto-reply/continuation/post-compaction-release.ts` already owns the same
post-compaction lifecycle. Moving these three functions there, updating callers
directly, and retaining lazy imports for heavy dispatch is a bounded candidate
pending GitNexus cycle/impact proof. The remainder of the shared file should not
be split.
...
| Check | Continuation parser -> existing `continuation/signal.ts` | Compaction completion -> existing `continuation/post-compaction-release.ts` |
| --- | --- | --- |
