...
| `pnpm check`                                                                                                                                                                    | PASS                    | Passed before the targeted test gate; reran and passed after the script routing fix changed `scripts/test-projects.test-support.mjs`.                                                                                                                                                          |
| `pnpm test src/auto-reply src/agents/tools/request-compaction-tool.test.ts src/agents/tools/continuation-tools-registration.test.ts src/config/zod-schema.continuation.test.ts` | DESIGN-BREAK, then PASS | First run routed exact `src/auto-reply` to the generic unit shard, whose excludes removed all auto-reply tests. Patched exact-root routing to the auto-reply shard, added `test/scripts/test-projects.test.ts` coverage, and reran successfully.                                               |
| `pnpm build`                                                                                                                                                                    | PASS                    | Build completed on the v5.2 candidate tree after the merge repairs and test-routing fix.                                                                                                                                                                                                       |
| `pnpm test src/infra/heartbeat src/agents/heartbeat-system-prompt.test.ts src/auto-reply/heartbeat src/auto-reply/heartbeat-filter.test.ts`                                     | PASS                    | Exact workorder command exited green. Because `src/infra/heartbeat` and `src/auto-reply/heartbeat` are historical directory operands in the v52 tree, also ran the current explicit heartbeat file set from `git ls-files`; that supplemental proof passed.                                    |
