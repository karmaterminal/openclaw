...
import { G as validateExecApprovalResolveParams, U as validateExecApprovalGetParams, W as validateExecApprovalRequestParams, _t as validateRequestFrame, an as errorShape, cn as getPreauthHandshakeTimeoutMsFromEnv, in as ErrorCodes, k as validateConnectParams, ln as buildDeviceAuthPayload, mt as validatePluginApprovalResolveParams, pt as validatePluginApprovalRequestParams, r as formatValidationErrors, un as buildDeviceAuthPayloadV3, vt as validateSecretsResolveParams, yt as validateSecretsResolveResult } from "./client-BbNZvLKI.js";
...
import { i as normalizeCronJobInput } from "./openclaw-tools-B7O1T0Lz.js";
...
import { n as collectCommandSecretAssignmentsFromSnapshot } from "./command-secret-gateway-XfhFdAGi.js";
...
import { i as failTaskRunByRunId, r as createRunningTaskRun, t as completeTaskRunByRunId } from "./detached-task-runtime-CAYAxYN7.js";
...
import { c as initSubagentRegistry } from "./subagent-registry-CrV0WlFt.js";
...
import { t as createPluginRuntime } from "./runtime-vGcLm8Uk.js";
import { o as getTotalPendingReplies } from "./dispatch-D3T9JIUr.js";
...
import { n as createChannelApprovalHandlerFromCapability } from "./approval-handler-runtime-DbnDDA0L.js";
...
import { i as safeParseJson } from "./nodes.helpers-D8D4C1XE.js";
...
import { t as closeMcpLoopbackServer } from "./mcp-http-D5zGKrLH.js";
...
import { C as TICK_INTERVAL_MS, S as MAX_PREAUTH_PAYLOAD_BYTES, _ as DEDUPE_MAX, b as MAX_BUFFERED_BYTES, f as isSuppressedControlReplyLeadFragment, m as abortChatRunById, p as isSuppressedControlReplyText, v as DEDUPE_TTL_MS, x as MAX_PAYLOAD_BYTES, y as HEALTH_REFRESH_INTERVAL_MS } from "./chat-28kBR2F7.js";
...
import { t as getHealthSnapshot } from "./health-BEP__pBN.js";
import { t as runSetupWizard } from "./setup-B699yQAf.js";
import { d as startTaskRegistryMaintenance, f as stopTaskRegistryMaintenance, g as markCronJobActive, h as clearCronJobActive, n as getInspectableTaskRegistrySummary } from "./task-registry.maintenance-DvQdNVMn.js";
...
async function loadCronExecutorRuntime() {
	cronExecutorRuntimePromise ??= import("./run-executor.runtime-Ca0uf6-7.js");
...
async function loadCronDeliveryRuntime() {
	cronDeliveryRuntimePromise ??= import("./run-delivery.runtime-3KFvFbCI.js");
...
async function loadCliRunnerRuntime() {
	return await import("./cli-runner.runtime-Drpn4JtB.js");
}
...
function recoverPendingSessionDeliveries(params) {
	setTimeout(() => {
		(async () => {
			const { recoverPendingRestartContinuationDeliveries } = await import("./server-restart-sentinel-_8VFYYnI.js");
...
function getManagedImageAttachmentsModule() {
	managedImageAttachmentsModulePromise ??= import("./managed-image-attachments-BwUZKpr1.js");
...
function getOpenAiHttpModule() {
	openAiHttpModulePromise ??= import("./openai-http-D18s36Po.js");
	return openAiHttpModulePromise;
}
function getOpenResponsesHttpModule() {
	openResponsesHttpModulePromise ??= import("./openresponses-http-XgzCCigr.js");
	return openResponsesHttpModulePromise;
}
function getSessionHistoryHttpModule() {
	sessionHistoryHttpModulePromise ??= import("./sessions-history-http-QGIT0bnV.js");
	return sessionHistoryHttpModulePromise;
}
function getSessionKillHttpModule() {
	sessionKillHttpModulePromise ??= import("./session-kill-http-D2DuYkz2.js");
	return sessionKillHttpModulePromise;
}
function getToolsInvokeHttpModule() {
	toolsInvokeHttpModulePromise ??= import("./tools-invoke-http-fNiZk3lG.js");
	return toolsInvokeHttpModulePromise;
}
function getVoiceClawRealtimeUpgradeModule() {
	voiceClawRealtimeUpgradeModulePromise ??= import("./upgrade-t7Nv_5K4.js");
...
async function startGatewaySidecars(params) {
		...
		try {
			...
			for (const sessionsDir of sessionDirs) {
				...
				if (result.cleaned.length > 0) {
					const { markRestartAbortedMainSessionsFromLocks } = await import("./main-session-restart-recovery-DADjEFnE.js");
	...
	if (params.cfg.acp?.enabled) {
		const [{ getAcpSessionManager }, { ACP_SESSION_IDENTITY_RENDERER_VERSION }] = await Promise.all([import("./manager-DD9V6FzI.js"), import("./session-identifiers-BYvkuRbw.js")]);
	...
	await measureStartup(params.startupTrace, "sidecars.restart-sentinel", async () => {
		...
		setTimeout(() => {
			import("./server-restart-sentinel-_8VFYYnI.js").then(({ scheduleRestartSentinelWake }) => scheduleRestartSentinelWake({ deps: params.deps })).catch((err) => {
	...
	await measureStartup(params.startupTrace, "sidecars.subagent-recovery", async () => {
		const { scheduleSubagentOrphanRecovery } = await import("./subagent-registry-CgrnzFLI.js");
		scheduleSubagentOrphanRecovery();
	});
	await measureStartup(params.startupTrace, "sidecars.main-session-recovery", async () => {
		const { scheduleRestartAbortedMainSessionRecovery } = await import("./main-session-restart-recovery-DADjEFnE.js");
	...
	try {
		...
		if (!minimalTestGateway) {
			if (deferredConfiguredChannelPluginIds.length > 0) {
				const { reloadDeferredGatewayPlugins } = await import("./server-plugin-bootstrap-Dc760FMV.js");
