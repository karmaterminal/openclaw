...
import { c as formatPluginCompatibilityNotice, n as buildPluginCompatibilityNotices } from "./status-DjbltF7p.js";
...
function loadAuthChoiceModule() {
	authChoiceModulePromise ??= import("./auth-choice-CsT27WrO.js");
...
function loadModelPickerModule() {
	modelPickerModulePromise ??= import("./model-picker-BXRza-T_.js");
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const { resolvePluginProviders, resolveProviderPluginChoice } = await import("./provider-auth-choice.runtime-C3vSpxuK.js");
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-CzWYs08G.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-DN9BfImr.js");
		const { applySkipBootstrapConfig } = await import("./onboard-config-Dx8ljnZ6.js");
	...
	const { applyLocalSetupWorkspaceConfig, applySkipBootstrapConfig } = await import("./onboard-config-Dx8ljnZ6.js");
	...
	if (authChoiceFromPrompt) {
		...
		({promptAuthChoiceGrouped} = await import("./auth-choice-prompt-CXCz2LKt.js"));
	...
	while (true) {
		...
		if (authChoice === "custom-api-key") {
			const { promptCustomApiConfig } = await import("./onboard-custom-CCr40FUd.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-Bn9dAWpZ.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { setupChannels } = await import("./onboard-channels-BC6nTOjn.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-BERns3Sx.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-C2l-AIqc.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	if (flow !== "quickstart") {
		const { setupPluginConfig } = await import("./setup.plugin-config-DjfZPy75.js");
	...
	const { setupInternalHooks } = await import("./onboard-hooks-D1rYQ5sE.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-BSe63B7_.js");
