...
import { a as resolveDiscordComponentEntry, o as resolveDiscordModalEntry, t as editDiscordComponentMessage } from "./send.components-DdIREvRT.js";
import { c as setPresence, i as unregisterGateway, r as registerGateway } from "./gateway-registry-BKG4KIVC.js";
import { B as BaseMessageInteractiveComponent, D as Container, F as Separator, I as StringSelectMenu, L as TextDisplay, N as Row, S as Modal, T as Button, _ as Message, a as MessageReactionRemoveListener, c as ThreadUpdateListener, d as Plugin, g as Guild, h as CommandWithSubcommands, i as MessageReactionAddListener, kt as overwriteApplicationCommands, m as Command, n as InteractionCreateListener, o as PresenceUpdateListener, p as RateLimitError, r as MessageCreateListener, s as ReadyListener, t as discord_exports, u as Client, v as User } from "./discord-dGuI-41N.js";
import { B as withValidatedDiscordProxy, I as DISCORD_REST_TIMEOUT_MS, L as createDiscordRequestClient, M as createDiscordRestClient, R as resolveDiscordProxyFetchForAccount, z as validateDiscordProxyUrl } from "./send.shared-CtymBXYp.js";
import { i as summarizeDiscordResponseBody, r as isDiscordRateLimitResponseBody } from "./api-dvAlVJCn.js";
import { n as formatDiscordUserTag, t as formatDiscordReactionEmoji } from "./format-D8TsaXxW.js";
import { _ as shouldEmitDiscordReactionNotification, a as resolveDiscordAllowListMatch, c as resolveDiscordChannelPolicyCommandAuthorizer, d as resolveDiscordMemberAccessState, g as resolveGroupDmAllow, i as normalizeDiscordSlug, n as isDiscordGroupAllowedByPolicy, p as resolveDiscordOwnerAccess, r as normalizeDiscordAllowList, s as resolveDiscordChannelConfigWithFallback, u as resolveDiscordGuildEntry } from "./allow-list-DJQxodPK.js";
import { t as formatMention } from "./mentions-BUtYuxA_.js";
import { _ as parseDiscordModalCustomIdForInteraction, g as parseDiscordModalCustomId, h as parseDiscordComponentCustomIdForInteraction, m as parseDiscordComponentCustomId } from "./components-DKVwdxD_.js";
import { i as isDiscordExecApprovalClientEnabled, n as getDiscordExecApprovalApprovers } from "./approval-shared-Cij_IvL1.js";
import "./approval-native-BmBdfFqP.js";
import { t as resolveDiscordConversationIdentity } from "./conversation-identity-BQIVhUQT.js";
import { t as resolveDiscordChannelAllowlist } from "./resolve-channels-DR-4uT8T.js";
import { t as resolveDiscordUserAllowlist } from "./resolve-users-xsFa-oiB.js";
import { a as isThreadArchived, d as formatThreadBindingDurationLabel } from "./thread-bindings.discord-api-j-S3mXSC.js";
import { r as parseApplicationIdFromToken, t as fetchDiscordApplicationId } from "./probe-DkBVeOhr.js";
import { o as raceWithTimeout, s as withAbortTimeout } from "./timeouts-WMG3-NWo.js";
import { c as resolveDiscordChannelInfo, d as resolveDiscordChannelInfoSafe, f as resolveDiscordChannelNameSafe, h as resolveDiscordChannelTopicSafe, p as resolveDiscordChannelParentIdSafe, u as resolveDiscordChannelIdSafe } from "./message-utils-DTDS2LpC.js";
import { i as resolveDiscordThreadParentInfo } from "./threading-DLucgrqB.js";
import { i as resolveDiscordEffectiveRoute, n as resolveDiscordBoundConversationRoute, o as handleDiscordDmCommandDecision, s as resolveDiscordDmCommandAccess } from "./route-resolution-BNtrhX88.js";
import { t as resolveDiscordSenderIdentity } from "./sender-identity-VIW3S0ks.js";
import { n as buildDiscordInboundAccessContext, t as buildDiscordGroupSystemPrompt } from "./inbound-context-DK8HTAwL.js";
import { t as authorizeDiscordVoiceIngress } from "./access-CjW29LFd.js";
import { n as buildDirectLabel, r as buildGuildLabel, t as deliverDiscordReply } from "./reply-delivery-eLTquvRi.js";
import "./approval-handler.runtime-Bk5aySHv.js";
...
async function dispatchDiscordNativeAgentReply(params) {
	...
	if (params.suppressReplies || didReply || dispatchResult.counts.final !== 0 || dispatchResult.counts.block !== 0 || dispatchResult.counts.tool !== 0) return;
...
var GatewaySendLimiter = class {
	...
	getStatus() {
		...
		return {
			remainingEvents: Math.max(0, 120 - this.outboundSendTimestamps.length),
	...
	canSend(now) {
		this.pruneWindow(now);
		return this.outboundSendTimestamps.length < 120;
	}
	...
	scheduleFlush() {
		...
		this.pruneWindow(now);
		const oldest = this.outboundSendTimestamps[0] ?? now;
		const delayMs = this.outboundSendTimestamps.length >= 120 ? Math.max(0, oldest + GATEWAY_SEND_WINDOW_MS - now) : 0;
...
var GatewayPlugin = class extends Plugin {
	...
	handlePayload(payload, resume) {
		...
		switch (payload.op) {
			case GatewayOpcodes.Hello:
				...
				else this.identifyWithConcurrency();
				break;
	...
	scheduleReconnect(resume, closeCode) {
		if (!this.shouldReconnect) return;
		this.stopHeartbeat();
		this.stopReconnectTimer();
		this.ws?.close();
		...
		const delay = Math.min(3e4, 1e3 * 2 ** Math.min(this.reconnectAttempts, 5));
...
function wrapDeployRestMethod(params) {
	return async (path, data, query) => {
		...
		try {
			...
			params.runtime.error?.(`discord startup [${params.accountId}] native-slash-command-deploy-rest:${params.method}:error ${Math.max(0, Date.now() - params.startupStartedAt)}ms path=${path} requestMs=${Date.now() - startedAt} error=${formatErrorMessage$1(err)}${details}`);
			throw err;
		}
	};
}
function installDeployRestLogging(params) {
	...
	for (const method of Object.keys(original)) params.rest[method] = wrapDeployRestMethod({
		method,
		original,
		runtime: params.runtime,
		accountId: params.accountId,
	...
	try {
		for (let attempt = 1; attempt <= maxAttempts; attempt += 1) try {
			...
			if (retryAfterMs > maxRetryDelayMs) {
				params.runtime.log?.(warn(`discord: native slash command deploy skipped for ${accountId}; retry_after=${retryAfterMs}ms exceeds startup budget. Existing slash commands stay active. Message send/receive is unaffected.`));
				return;
			}
			if (params.shouldLogVerbose()) params.runtime.log?.(`discord startup [${accountId}] deploy-retry ${Math.max(0, Date.now() - startupStartedAt)}ms attempt=${attempt}/${maxAttempts - 1} retryAfterMs=${retryAfterMs} scope=${err.scope ?? "unknown"} code=${err.discordCode ?? "unknown"}`);
			await sleep(retryAfterMs);
		}
	} catch (err) {
		const details = formatDiscordDeployErrorDetails(err);
		params.runtime.log?.(warn(`discord: native slash command deploy warning (not message send): ${formatErrorMessage$1(err)}${details}`));
	} finally {
...
async function loadConversationRuntime$1() {
	conversationRuntimePromise$1 ??= import("./agent-components.runtime-Cp0dFYng.js");
...
async function loadTypingRuntime() {
	typingRuntimePromise ??= import("./typing-B3zrWw57.js");
...
async function loadComponentsRuntime() {
	componentsRuntimePromise ??= import("./components-DKVwdxD_.js").then((n) => n.t);
...
async function loadDiscordVoiceRuntime() {
	const promise = discordVoiceRuntimePromise ?? import("./manager.runtime-WaBybxUK.js");
...
async function loadDiscordProviderSessionRuntime() {
	const promise = discordProviderSessionRuntimePromise ?? import("./provider-session.runtime-RtWVtcMw.js");
