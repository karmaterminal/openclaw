...
import { l as formatPluginCompatibilityNotice, r as buildPluginCompatibilitySnapshotNotices } from "./status-DlA41-uQ.js";
import { n as commitConfigWriteWithPendingPluginInstalls } from "./plugins-install-record-commit-Bg9eyegq.js";
...
async function detectSetupMigrationSources(params) {
	const [{ resolvePluginMigrationProviders }, { createMigrationLogger }, { resolveStateDir }] = await Promise.all([
		import("./migration-provider-runtime-BgMNPWBY.js"),
		import("./context-DvQzHSr9.js"),
...
async function selectSetupMigrationProvider(params) {
	const { resolvePluginMigrationProvider, resolvePluginMigrationProviders } = await import("./migration-provider-runtime-BgMNPWBY.js");
...
async function runSetupMigrationImport(params) {
	const [{ applyLocalSetupWorkspaceConfig, applySkipBootstrapConfig }, { createMigrationLogger, buildMigrationReportDir }, { createPreMigrationBackup }, { assertApplySucceeded, assertConflictFreePlan, formatMigrationPlan }, { resolveStateDir }, onboardHelpers] = await Promise.all([
		import("./onboard-config-DEIXtzp9.js"),
		import("./context-DvQzHSr9.js"),
		import("./apply-tyhzRTay.js"),
		import("./output-D2f44ms5.js"),
		import("./paths-BMxaLwkU.js"),
		import("./onboard-helpers-Dhkaze0n.js")
	]);
...
function loadAuthChoiceModule() {
	authChoiceModulePromise ??= import("./auth-choice-RRz4alHY.js");
	return authChoiceModulePromise;
}
function loadConfigLoggingModule() {
	configLoggingModulePromise ??= import("./logging-CTro4kdy.js");
	return configLoggingModulePromise;
}
function loadModelPickerModule() {
	modelPickerModulePromise ??= import("./model-picker-BdT2AAnW.js");
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const [{ resolveManifestProviderAuthChoice }, { resolvePluginSetupProvider }] = await Promise.all([import("./provider-auth-choices-Dhxz6zt1.js"), import("./setup-registry-Bz7YhHl4.js")]);
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-Dhkaze0n.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-ivOj89Xd.js");
		const { applySkipBootstrapConfig } = await import("./onboard-config-DEIXtzp9.js");
	...
	const { applyLocalSetupWorkspaceConfig, applySkipBootstrapConfig } = await import("./onboard-config-DEIXtzp9.js");
	...
	if (authChoiceFromPrompt) {
		...
		({promptAuthChoiceGrouped} = await import("./auth-choice-prompt-BMYswb3z.js"));
	...
	while (true) {
		...
		if (authChoice === "custom-api-key") {
			const { promptCustomApiConfig } = await import("./onboard-custom-CboUS4Xe.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-C6iFHlAg.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-dR6fOOi2.js");
		const { setupChannels } = await import("./onboard-channels-CoWycOHx.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-BGwJsgA5.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-DzHA6F6w.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	if (flow !== "quickstart") {
		const { setupPluginConfig } = await import("./setup.plugin-config-BPKMEF67.js");
	...
	const { setupInternalHooks } = await import("./onboard-hooks-CISp6VNY.js");
	...
	nextConfig = await writeWizardConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-Dcszf4EN.js");
